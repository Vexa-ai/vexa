...
- **Goal vs objective.** The **goal** is the destination — the *end of the plan* (the release). An **objective** is the *current go* — the one waypoint you execute toward now; the plan is the ordered path of objectives to the goal. The objective is the **unit of execution and assessment**: you are always executing toward exactly **one open objective** (never drifting), and it closes either **expected** (result met it → take the next objective, still in execution mode, autonomously) or **unexpected** (result diverged → **stop and interpret it *with the human*, as learning** — root-cause to a gap, codify, re-plan). Expected closure flows on its own; unexpected closure pulls in the human (ADR-0017).
