...
> **Apple Silicon (M1/M2/M3/M4) note:** Vexa Lite images are now published as multi-arch (`linux/amd64,linux/arm64`) starting with v0.10.6.1 — Apple Silicon machines pull the native arm64 layer automatically. Earlier image tags are amd64-only and require Rosetta 2 emulation, which adds CPU and memory overhead. If you're on Apple Silicon, pin to `:latest` (or any tag from v0.10.6.1+) for the native build. Closes [#321](https://github.com/Vexa-ai/vexa/issues/321).
...
<ResponseField name="TRANSCRIPTION_SERVICE_URL" type="string" required>
  Transcription service endpoint.

  Default: `https://transcription.vexa.ai/v1/audio/transcriptions`
</ResponseField>

<ResponseField name="TRANSCRIPTION_SERVICE_TOKEN" type="string" required>
  API key for the transcription service. Get yours from [vexa.ai/account](https://vexa.ai/account).
</ResponseField>
...
Vexa ships with **two TTS providers** that callers select per-request:

| Provider | Engine | Network calls | API key needed | Notes |
|---|---|---|---|---|
| `piper` *(default)* | [Piper TTS](https://github.com/rhasspy/piper) — local, ONNX, CPU | None (voices auto-download from HuggingFace on first use) | No | ~30 languages out of the box. Auto-detects input language and picks the matching voice. |
| `openai` | OpenAI's `/v1/audio/speech` | Yes — calls `api.openai.com` | **Yes — `OPENAI_API_KEY`** | Higher quality on supported voices; counts against your OpenAI usage. |

Callers (`POST /speak` from agent-api / dashboard) choose the provider via the `provider` field. Default is `piper`.

<ResponseField name="OPENAI_API_KEY" type="string">
  Required only when callers select `provider=openai` for `/speak`. The default `provider=piper` runs locally and ignores this variable. Without `OPENAI_API_KEY` set, requests with `provider=openai` return `503 — TTS provider unavailable`; the bundled local Piper provider continues to work.
</ResponseField>

<ResponseField name="PIPER_VOICES_DIR" type="string" default="/app/voices">
  Filesystem location for downloaded Piper voice models. Mount a volume here for persistence across container restarts (otherwise voices re-download on first request after restart, ~25–60 MB per language).
</ResponseField>

<ResponseField name="PIPER_DEFAULT_VOICES" type="string" default="en_US-amy-medium,en_US-danny-low">
  Comma-separated Piper voice names to pre-load on startup. Other languages are downloaded lazily when first requested via auto-detection or an explicit `voice` parameter.
</ResponseField>
...
- [api-gateway](https://github.com/Vexa-ai/vexa/blob/main/services/api-gateway/README.md) -- REST/WebSocket API
- [meeting-api](https://github.com/Vexa-ai/vexa/blob/main/services/meeting-api/README.md) -- bot lifecycle orchestration
- [dashboard](https://github.com/Vexa-ai/vexa/blob/main/services/dashboard/README.md) -- web UI
- [admin-api](https://github.com/Vexa-ai/vexa/blob/main/services/admin-api/README.md) -- user and token management
