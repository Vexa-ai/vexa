...
The interactive bot uses the same session model: all interactions happen within a single `session_uid`. Chat messages captured by the bot can optionally be injected into the transcription stream.

Full details: [Interactive Bots Guide](interactive-bots)
