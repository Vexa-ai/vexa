...
When a bot has interactive capabilities enabled (default), the following additional events are published on the WebSocket connection:
...
These events are also published on the Redis channel `va:meeting:{meeting_id}:events`. See the [Interactive Bots guide](interactive-bots) for full details on controlling the bot's voice, chat, and screen share capabilities.
