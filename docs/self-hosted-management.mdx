...
**Before starting**, ensure Vexa services are running - see the [Deployment Guide](deployment) for setup instructions.
...
Admin can also set webhooks for users via `PUT /user/webhook` on the user API (port 8056) with the user's API key. See [Settings API](api/settings#put-userwebhook).
