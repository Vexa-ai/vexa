...
clone. `make probe` then runs the whole journey — ready → spawn → schedule → boot → join → transcribe →
live-view → stop → log sweep — in nine stages with a per-stage verdict, against any of the three. What we do **not** ship: a backup or
