...
- **Wrong URL kind** — you need the **secret ICS address** (Google: *Secret address in iCal
  format*; Outlook: the published **ICS** link), not the calendar's public/share URL. Reads via
  `GET /user/calendar` show the connected feed masked.
...
- **Patience, then the interval** — the sweep runs every `CALENDAR_SYNC_INTERVAL_S` (default 5
  min). On a self-host also confirm `ADMIN_API_URL` and `INTERNAL_API_SECRET` are set for
  meeting-api — without them the sync loop is disabled.
- **Feed too large / unreachable** — feeds over 2 MB or behind redirects are refused; the sync
  state records the fetch error.
