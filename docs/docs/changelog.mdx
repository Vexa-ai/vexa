...
|---|---|---|
| Mid-call bot config — `PUT /bots/{platform}/{id}/config` (change language/task live) | wired ([#516](https://github.com/Vexa-ai/vexa/issues/516)) | Served: meeting-api publishes an `acts.v1` `reconfigure` on the bot's command channel and the running bot applies it to its live STT config — the next transcription request carries the new `language`/`task`, no respawn. Takes effect from the next utterance. Wire shape in the [Meetings API](/api/meetings#manage-the-bot) reference. |
