...
| **Voice agent — TTS/audio speak** | ✅ `speak`, `speak_audio`, and `speak_stop` dispatch to the PulseAudio virtual mic; module-tested | 🟡 wired + module-tested — authenticated active-meeting lookup publishes acts.v1; PCM is normalized to WAV; POST and DELETE are proxied | ❌ none | User-reachable in code, but **not yet claimed live-proven** on this change. A Lite Google Meet acceptance run remains the final witness for [#514](https://github.com/Vexa-ai/vexa/issues/514). See [Meetings API](/api/meetings). |
...
| Voice agent / TTS speak (`POST/DELETE …/speak`) | service-wired + module-tested; live Google Meet witness pending for [#514](https://github.com/Vexa-ai/vexa/issues/514) |
