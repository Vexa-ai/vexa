...
(`touch` / idle-enforcer). **M2 — the [live meeting](/how-to/live-copilot) feed — is built and proven
live**; the in-product copilot that once rode it has been removed in favour of one intelligence, the
agent (see the truth table below).
...
|---|---|---|---|---|
| **Live meeting feed** (the transcript as the call runs) | ✅ built + proven live | ✅ `GET /api/meeting/stream` — owner-scoped since the cross-tenant fix ([#677](https://github.com/Vexa-ai/vexa/issues/677)) | ✅ the meeting tab renders the live transcript | **Removed:** the in-product meeting copilot that shared this feed (cleaned notes + proactive cards, `POST /api/meeting/{start,process}`, [ADR-0027](https://github.com/Vexa-ai/vexa/blob/main/docs/adr/0027-meeting-processing-ssot.md), now superseded). Vexa runs no model calls of its own beside the agent; ask about a meeting in the chat. The [how-to](/how-to/live-copilot) is accurate. |
