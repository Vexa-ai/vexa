...
| **User webhooks** (your URL, signed events) | — | ✅ live — self-serve config (URL/secret/events, secret-masked), the `webhook.v1` HMAC delivery module with retry queue, and the queryable per-user [delivery ledger](/webhooks#delivery-history) | ❌ no settings UI | Deliveries reach real external receivers on the hosted service and are recorded per attempt with the receiver's own status code (verified 2026-08-18 via `GET /user/webhook/deliveries`). Exactly-once emission and outage-durable retry remain open as [#519](https://github.com/Vexa-ai/vexa/issues/519)/[#520](https://github.com/Vexa-ai/vexa/issues/520). `PUT /user/webhook` still answers with the whole user record — [#1241](https://github.com/Vexa-ai/vexa/issues/1241). |
...
| **MCP server** | — | ✅ live on compose and the hosted service, fronted by the gateway at `/mcp` — `POST` buffered, `GET` (the SSE stream) relayed unbuffered ([#795](https://github.com/Vexa-ai/vexa/issues/795)) | n/a | Proven end-to-end against `https://api.cloud.vexa.ai/mcp` on 2026-08-18: `initialize` → `tools/list` (9 tools) → `prompts/list` (4 prompts), caller credential forwarded — the proof [#888](https://github.com/Vexa-ai/vexa/issues/888) was waiting for. `update_bot_config` wraps a route that is still `404`. **The open-source Helm chart deploys no MCP service** — the hosted service runs it on Kubernetes through the separate platform chart ([MCP server](/vexa-mcp#kubernetes-and-helm)). |
