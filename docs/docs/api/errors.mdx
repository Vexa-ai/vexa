...
- `GET /transcripts/*` → `400` on any query parameter other than `since`, naming it
  (`detail: "Unsupported query parameter(s): limit. This endpoint accepts: since."`), and `400` on a
  `since` that isn't an ISO-8601 UTC timestamp. A parameter the endpoint does not honour is refused,
  never accepted and dropped — see [following a live meeting](/api/meetings#following-a-live-meeting-since).
