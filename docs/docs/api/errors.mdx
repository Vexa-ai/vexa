...
1. **Per-IP edge throttle (pre-auth).** An optional fastapi-guard edge layer caps requests per client IP
   *before* the API key is validated — so an IP flooding invalid keys, or rotating many keys from one IP
   ...
   that keeps offending past a threshold is **auto-banned for a window**. Default: **OFF**
   (`GUARD_ENABLED=false`); set `GUARD_ENABLED=true` + `GUARD_RATE_LIMIT_RPM` to turn it on. Whether
   the hosted service runs this layer is a deployment setting and is not published here.
