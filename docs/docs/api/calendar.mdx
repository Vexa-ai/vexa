...
platform/native ID, and the constructed meeting URL. Complete multi-calendar provenance is under
    ...
    "calendar_sources": [
      {
        ...
        "event": {
          ...
          "calendar": {"name": "VCALENDAR", "properties": {}},
          "component": {"name": "VEVENT", "properties": {}},
          "series_master": {"name": "VEVENT", "properties": {}}
        }
      }
    ]
  }
...
`event` preserves every ICS property and parameter, repeated property, provider extension, and
nested component in JSON-safe canonical text. `series_master` appears only for a selected recurring
override. The configured secret feed URL is explicitly redacted and is never added as provenance.
