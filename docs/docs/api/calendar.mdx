...
Background cadence is controlled by `CALENDAR_SYNC_INTERVAL_S`, default 300 seconds — the value the
hosted service runs today. Provider publication delay is additional. See
...
**List rows** (`GET /meetings`, `GET /bots`) project each source to four keys:
...
**Detail reads** (`GET /meetings/{meeting_id}`, `GET /transcripts/by-id/{meeting_id}`) return two
more keys per source: `uid`, the provider's event UID, and `event`, the stored ICS snapshot —
`resolved_start` plus the `calendar` and `component` property maps, which include the event's
`SUMMARY` and `DESCRIPTION`. Treat a meeting detail response as carrying calendar content, and
scope who may read it accordingly.

The configured secret feed URL is redacted everywhere and never appears as provenance.
See [Calendar sync](/how-to/calendar-sync) for the setup walkthrough.
