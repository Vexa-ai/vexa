...
<ParamField body="skip_probe" type="boolean">
  Set to `true` to save without probing. Defaults to `false`; applies only to this request and is
  never stored. Use when the endpoint cannot be tested from admin-api; reachability remains unverified.
</ParamField>

Same partial-update and clearing rules as [model configuration](#model-configuration).

Before storing an update that leaves a customer URL configured, admin-api probes that URL with
the resulting token, including on token-only updates. It POSTs the canonical one-second 16 kHz
mono 16-bit PCM WAV tone to `/v1/audio/transcriptions`, appending the path only if absent, with
`model=whisper-1`, `response_format=json`, and Bearer authentication when a token is set.
The timeout is 10 seconds. HTTP 2xx with a JSON object containing a string `text` passes.

The **200** PUT response is the GET shape plus `probe`: `"ok"`, `"skipped"` when `skip_probe`
is true, or `null` if no customer URL remains. GET exposes the saved verdict in `probe_status`
and its UTC timestamp in `probe_at` (ISO-8601, second precision, ending in `Z`). These are `null`
when unstamped. Clearing the URL removes both stamps; a token without a URL is not probed.

A failed probe returns **422** and leaves the stored configuration and stamps unchanged:

```json Response — 422
{
  "probe": "unauthorized",
  "detail": "https://stt.example.com/v1/audio/transcriptions: HTTP 401"
}
```

`probe` is `"unauthorized"` for HTTP 401/403, or `"unreachable"` for network errors, timeouts,
other non-2xx statuses (including 404), non-JSON bodies, or a missing/non-string `text` field.
Details redact the token and limit any response-body excerpt to 200 characters. URL validation
runs first and retains the ordinary `{"detail":"…"}` error shape.
