...
The account's LLM settings, used by the [agent control plane](/api/agent). There is one model —
the agent's — so there is one model dial.
...
<ParamField body="model" type="string">
  Model name for agent work. A name outside the deployment's allowlist is dropped back to the
  deployment default at dispatch time rather than rejected here.
</ParamField>
...
<ParamField body="extra_body" type="string">
  Request fields the OpenAI dialect cannot express, as a JSON string — a self-hosted vLLM/Qwen
  endpoint returns no valid JSON until thinking is disabled with
  `{"chat_template_kwargs": {"enable_thinking": false}}`. Write-only: it is stored and applied, and
  the read-back above does not echo it.
</ParamField>
<ParamField body="effort" type="string">
  The reasoning-effort pin — `low`, `medium`, `high` or `xhigh`. Unset leaves the harness's own
  default.
</ParamField>
<ParamField body="runner" type="string">
  Which harness runs this account's workspace turns: `claude-code`, `codex` or `openai-agent`. Stored
  as an opaque slug — an unknown one is dropped back to the deployment default at dispatch, not
  refused here.
</ParamField>

Only three things return **422** on this write: a `mode` outside `subscription`/`custom`, a `base_url`
that is not an `http(s)` URL with a hostname, and any field over 2,048 characters. `effort` and
`runner` are not validated against a list at this tier, and an unrecognized field name is ignored
rather than refused.
...
removes the account override entirely.
...
| `422` | model, transcription, webhook writes, admin patch | Invalid `mode`, a non-`http(s)` URL, a string over 2,048 characters, an out-of-range `limit`, or a field the admin patch does not recognize. |
