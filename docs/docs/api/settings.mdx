...
<Note>
Webhook **delivery** is proven live (an owner-witnessed production walk, 2026-07-19, one POST per
event at an external receiver); the user-facing **Delivery History** is not — it lists none of them
([#841](https://github.com/Vexa-ai/vexa/issues/841)). See [Webhooks → What is proven, and what is
not](/webhooks#what-is-proven-and-what-is-not). The configuration endpoints below are the settings
surface for it; they store what you send.
</Note>
