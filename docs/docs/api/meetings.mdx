...
unambiguous or explicitly selected owned row and applies the same rules (`404` unknown/unowned, `409` while in flight). Use it
...
|---|---|
| `PATCH /meetings/{platform}/{native_meeting_id}` | **Yes** — requires `?meeting_id=` if multiple owned rows match. |
...
Native-keyed meeting writes (delete, patch, annotate, workspace, share, docs and intent) return `409` when multiple owned sessions share the key.
The response's `detail.candidates` lists each row's `id`, `created_at` and `status`.
Add `?meeting_id=<id>` to select one candidate; a nonmatching ID returns `404`.
