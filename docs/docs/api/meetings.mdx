...
For `telemost`, use the canonical `https://telemost.yandex.ru/j/<10-digit-id>` link. The API can
derive the platform and ID from that URL, or construct the canonical URL from an explicit
`platform: "telemost"` plus `native_meeting_id`.
...
<ParamField body="meeting_url" type="string">A Meet/Zoom/Teams/Jitsi/Telemost link — parsed server-side into `platform` + `native_meeting_id`. Unrecognized links are rejected with `422`.</ParamField>
...
<ParamField body="platform" type="string" required>`google_meet` · `zoom` · `teams` · `jitsi` · `telemost`</ParamField>
