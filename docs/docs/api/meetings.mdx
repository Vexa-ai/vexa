...
For `jitsi`, `native_meeting_id` is the room name and resolves against the public
**meet.jit.si**. A **self-hosted** Jitsi deployment lives on your own host, so pass the full
`meeting_url` (e.g. `https://jitsi.example.org/MyRoom`) alongside `platform: "jitsi"` — and to
make **calendar sync** recognize your deployment's links, list its hostnames in the
`VEXA_JITSI_HOSTS` env (comma-separated). On Jitsi the transcript also carries the meeting's
**chat**: each chat message arrives as a segment with `source: "chat"` (the sender is the
`speaker`), and speaker names come from the conference's own dominant-speaker signal.
