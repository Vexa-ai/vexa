...
`ramp_bots` limits new bots per account in a sliding window; `0` means no new bots.
`ramp_window_s` sets that window in seconds (at least `1`); failed attempts consume slots too; both dials are account settings, not request fields.
The default is 2 per 300 s, configured by `VEXA_RAMP_BOTS_DEFAULT` and `VEXA_RAMP_WINDOW_S_DEFAULT`.
A ramp refusal returns `429` with `Retry-After` naming the seconds until the next slot and a string `detail`.
