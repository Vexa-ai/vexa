...
`auto_join` here is the **global default** stamped onto imported meetings. `bot_name` is the
user-wide participant name read by the calendar auto-join scheduler (default `Vexa`).

```bash PUT /user/calendar
curl -X PUT "$API_BASE/user/calendar" \
  -H "X-API-Key: $API_KEY" -H "Content-Type: application/json" \
  -d '{"ics_url":"https://calendar.google.com/calendar/ical/…/basic.ics","auto_join":true,"bot_name":"My Meeting Notes"}'
```
...
```json Response
{ "ics_url_set": true, "ics_url_masked": "calendar.google.com/…s.ics", "auto_join": true, "bot_name": "My Meeting Notes" }
```
