...
The `speaker` field on these segments is also the only route to a participant list today — there is no
participants endpoint. See [Get a meeting's participants](/how-to/meeting-participants) for how to derive
a roster and what it leaves out.
...
unaffected and still returns every key that is present.

<Warning>
`speaker_events` and `participants` are **no longer written at all** — not since the 0.12 cutover
(2026-07-18), with no replacement table ([#861](https://github.com/Vexa-ai/vexa/issues/861), open). The
detail endpoint returns them when present, but on any recent meeting they are simply absent. Do not read
an attendance list off `data.participants`; see
[Get a meeting's participants](/how-to/meeting-participants).
</Warning>
