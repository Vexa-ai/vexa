...
Fifteen tools, plus four prompts (`vexa.meeting_prep` · `vexa.during_meeting` · `vexa.post_meeting` ·
...
| `report_issue` | files a ticket with the Vexa maintainers (the deployment's ticket sink) |
| `delete_meeting_artifacts` | `DELETE /meetings/{platform}/{native}` — **irreversible erasure** |

`update_bot_config` wraps a route that is not wired in the v0.12 open core and returns `404` — see
[Meetings API → Manage the bot](/api/meetings#manage-the-bot). The others reach live routes.

<Warning>
`delete_meeting_artifacts` destroys data and cannot be undone: the meeting's transcript, the notes
and share links derived from it, its recordings, and the captured-signal tape Vexa keeps for
debugging all go. Only a meeting that has ended can be erased; one still running answers `409`.
See [Meetings API → Edit a plan or delete meeting artifacts](/api/meetings).
</Warning>
