...
| **API** | each domain's published, gateway-fronted endpoints | meetings-control (`/bots`, `/meetings`, `/transcripts`, `/intent`, `/ws` status). agent-control (`/api/meeting/stream`, `/api/chat`, `/api/workspace/*`, `/api/models`) | hold business logic in the gateway; let a client reach a domain backend directly |
...
- follow a live meeting — `GET /api/meeting/stream` (the transcript SSE; there is no second, in-product
  processing lane to switch on — PRD decision 34)
...
- "agent on meeting" — one op = `POST /bots`, then a `POST /api/chat` turn carrying an `active`
  meeting reference (cookbook entry #2)
...
| **CP8** | cookbook "agent-on-meeting" = `POST /bots`, then a `POST /api/chat` turn carrying the meeting reference | wiring | meeting input → the bot call (right args) + state + partial-failure surfaced |
