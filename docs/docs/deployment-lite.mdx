...
The meeting bot's Chromium runs **sandboxed as a non-root user**, which needs the container to allow
user namespaces — `make lite` launches it with `--security-opt seccomp=unconfined` for you. If you run
the image yourself (a custom `docker run`, compose, or Dockge stack), add `security_opt:
[seccomp=unconfined]` (or `--security-opt seccomp=unconfined`), or bots fail to join with *"No usable
sandbox"*. To keep the sandbox off instead, set [`CHROME_NO_SANDBOX=1`](/configuration) — the bot then
