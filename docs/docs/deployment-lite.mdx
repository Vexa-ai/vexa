...
`make lite` (= `make -C deploy/lite all`) writes a minimal `.env` if you don't have one — minting a
random `ADMIN_TOKEN` into it — pulls `vexaai/vexa-lite:$IMAGE_TAG` (default `v012`; a locally built
`vexa-lite:dev` wins when present), starts the `vexa-lite-postgres` and `vexa-lite-minio` sidecars
on the `vexa-lite-net` Docker network, boots the app container, waits on the gateway health
endpoint, and probes the three front doors:
...
`make lite` **prints both credentials when it finishes**: the `ADMIN_TOKEN` from `.env` (it mints
users and API keys — treat it as root on this deployment) and the API key for that
`self-host@vexa.ai` user, ready to paste into a request. If the gateway never answers, `make lite`
**fails** and prints the app container's last log lines rather than reporting a success it did not
observe.
...
To run agents on your own Claude subscription without copying credentials into `.env`, point
`HOST_CLAUDE_CREDENTIALS` at the file and Lite mounts it read-only into the container:

```bash
make lite HOST_CLAUDE_CREDENTIALS=$HOME/.claude/.credentials.json
```

It is **opt-in** — settable on the command line, in the environment, or as a key in the repo-root
`.env`. Leave it unset and nothing is mounted, whatever exists on the host. This is the same
contract the [compose deployment](/deployment) has always had.

The MinIO sidecar images are `make` variables too: `MINIO_IMAGE` and `MINIO_MC_IMAGE`, defaulting
to MinIO's own registry (`quay.io/minio/...`), pinned to a dated release. Override them to use a
mirror or a different version.
