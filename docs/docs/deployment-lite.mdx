...
The probe mints a key and drives the whole journey — the same nine stages as every surface:
ready → spawn → schedule → boot → join → transcribe → live-view → stop → log sweep. Lite runs the **real** bot in-process, so a dead meeting URL ends in a truthful
