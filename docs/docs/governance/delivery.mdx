...
**D-R1a — An exploratory fork enters through intake, not delivery.** Forks are welcome
incubation: they can discover a symptom, collect evidence, and test a candidate shape without a
shipping promise. Before any fork is presented for upstream integration, its author or an agent
selects **fork intake** and emits a receipt: source, linked issue, one candidate value at a time,
target module/seam, existing harness or fixture range, human bar, and a disposition for each
concern (**already present · adapt · evidence only · obsolete · unsafe**). Intake is review-only:
whole-stack rebase, merge, and cherry-pick are invalid outcomes. An accepted concern is recut
from current main as a narrow prepared-issue delivery. · *Gate:* the PR contribution-mode receipt;
a PR whose head is a contributor fork must declare `fork intake` and carry the receipt before it
can satisfy the merge card.
...
| D-R1a fork intake | PR contribution-mode receipt; contributor-fork heads require the fork-intake receipt at merge card | **have** (CI: `merge-card-gate`) |
