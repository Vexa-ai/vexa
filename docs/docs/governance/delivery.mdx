...
4. **Credit** — release notes name authors **and** validators, per change (D11/D15).
...
Guarantees of every release: all value witnessed · no regression against the previous release
(the no-regression rows + batch validation) · artifact-pointer truth · the loop closed to every
reporter.
...
- **Floor met, awaiting validation** → drive it: ping the preferred signer (the reporter);
  ...
  owner-only leg → surface it to the owner explicitly, never let it wait silently.
...
**Step 4 — report facts.** End the cycle with the dashboard, not adjectives: PRs by state ·
