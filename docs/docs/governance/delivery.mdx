...
| **L3.5 — transcribes** | fixture audio → real pipeline → real CPU STT → **golden words** land via the API — no Meet, no human (committed counting fixture, 1..N oracle, per-stage attribution + STT-down negative control) | `pr-value-stt` / `validate-counting-stt` (`make -C core/meetings/eval counting-ci`) | PRs touching the transcription path; every release |
