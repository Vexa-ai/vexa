...
- **Encrypted at rest (✅)** — sealed by `secret_store` under one server-side key: `VEXA_SECRETS_KEY` when
  ...
  first time it is read. Still use a **minimally-scoped, revocable fine-grained PAT**: a stored PAT is
  password-equivalent, and encryption at rest does not protect a running server.
