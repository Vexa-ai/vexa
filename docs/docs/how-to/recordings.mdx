...
is still in flight). A storage error leaves the metadata intact so you can safely retry.

This route erases **one recording**. To erase everything a completed meeting left — its transcript,
the notes and share links derived from it, every recording, and the captured-signal tapes Vexa keeps
internally for debugging — use `DELETE /meetings/{meeting_id}`. That is the one to call when someone
asks for a meeting to be deleted; deleting the recordings alone leaves the transcript and the tapes.
