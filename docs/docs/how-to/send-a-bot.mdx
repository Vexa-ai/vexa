...
<Tabs>
  ...
  <Tab title="Jitsi Meet">
    ```bash
    # meet.jit.si room — the room name is the id
    curl -X POST "$API_BASE/bots" \
      -H "X-API-Key: $API_KEY" -H "Content-Type: application/json" \
      -d '{"platform":"jitsi","native_meeting_id":"MyRoom","bot_name":"Vexa"}'

    # self-hosted Jitsi — pass the full meeting_url on your deployment
    curl -X POST "$API_BASE/bots" \
      -H "X-API-Key: $API_KEY" -H "Content-Type: application/json" \
      -d '{"platform":"jitsi","native_meeting_id":"MyRoom","meeting_url":"https://jitsi.example.org/MyRoom","bot_name":"Vexa"}'
    ```
  </Tab>
</Tabs>
