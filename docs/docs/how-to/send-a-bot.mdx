...
```bash
# change the language mid-call — the German standup an English guest just joined
...
The bot stays in the call and applies the change to its **next** transcription window — the
utterance in flight finishes under the old setting. Send `{"language": null}` to drop back to
auto-detect, or `{"task":"translate"}` to render foreign speech as English text. Fields you omit
are left alone. Full request/response shape:
[Meetings API](/api/meetings#change-a-running-bots-language-or-task).
