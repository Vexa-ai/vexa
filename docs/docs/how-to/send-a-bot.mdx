...
description: "Capture a Google Meet, Zoom, Teams, Jitsi, or Yandex Telemost call in real time."
---
...
<Tabs>
  ...
  <Tab title="Yandex Telemost">
    ```bash
    curl -X POST "$API_BASE/bots" \
      -H "X-API-Key: $API_KEY" -H "Content-Type: application/json" \
      -d '{"platform":"telemost","native_meeting_id":"1111111111","meeting_url":"https://telemost.yandex.ru/j/1111111111","bot_name":"Vexa"}'
    ```
  </Tab>
</Tabs>

`native_meeting_id` is the id **inside** the join URL (e.g. `abc-defg-hij` for Meet, the room name for
Jitsi, or the numeric meeting ID after `/j/` for Telemost), not the whole URL.
...
On Meet, Teams, and Telemost the bot may wait in the lobby until a host admits it; a Jitsi room
with the lobby enabled behaves the same way.
