...
| `GUARD_BLOCK_CLOUD_PROVIDERS` | — | CSV of cloud providers to block by IP range, opt-in. By default, values are passed to guard-core exactly as written: guard-core 3.4.0 silently ignores an unrecognized or wrong-case name (a boot warning names it). With `GUARD_BLOCK_CLOUD_PROVIDERS_STRICT=true`, names are case-insensitive, normalized to `AWS`, `GCP`, or `Azure`, and any other name refuses to start. |
| `GUARD_BLOCK_CLOUD_PROVIDERS_STRICT` | `false` | Opt-in strict validation for `GUARD_BLOCK_CLOUD_PROVIDERS`: normalizes provider case and refuses to start on an unrecognized name. Off by default so an upgrade cannot silently flip a previously-inert entry (e.g. `aws`) into live enforcement, or crash-loop a boot on an already-set typo. |
