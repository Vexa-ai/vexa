...
| `VEXA_CSRC_INACTIVE_MS` | `800` | Microsoft Teams: how long the transport sensor holds a contributing source active after its last observed contribution before it synthesizes a deactivation. Forwarded to spawned bots. Usable range `100`–`10000`; a non-blank unusable value warns and falls back to `800`. |
