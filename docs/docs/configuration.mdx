...
runtime teardown and resumed after restart. Unavailable continuation checks receive a grace of
`unavailable_threshold` consecutive one-minute boundaries (default 3); any successful decision
resets the streak, and a threshold of 1 stops at the first unavailable boundary in enforce mode.
