...
is **auto-banned for a window**; sustained abuse costs the abuser, not the operator. The code default
is `GUARD_ENABLED=true`; the deploy surfaces set `GUARD_ENABLED=false`, so you opt in by overriding
it. Whether the hosted service runs this layer is a deployment setting and is not published here.
...
| Variable | Default | Purpose |
|---|---|---|
| `GUARD_ENABLED` | `true` (code) / `false` (deploy surfaces) | Kill switch for the entire edge layer. |
