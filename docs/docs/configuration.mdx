...
| `CALENDAR_MAX_ICS_BYTES` | `10485760` | Max size in bytes of a fetched ICS feed (10 MB); the body streams and is abandoned past this. A secret iCal address exports the whole calendar including history, so raise this if a real work calendar is refused as too large. Helm: `meetingApi.maxIcsBytes`. |
