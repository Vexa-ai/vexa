...
| `VEXA_AGENT_DEFAULT_SUBJECT` | `u_live` | Single-user fallback subject, used only when no gateway-resolved `X-User-Id` is present. |
| `VEXA_REQUIRE_GATEWAY_IDENTITY` | `1` on Lite, off elsewhere | Refuse any request that does not carry the gateway's `X-Gateway-Verified` marker, so identity headers are honoured only when the gateway injected them and the fallback above cannot be reached from outside. |
| `VEXA_AGENT_API_BIND` | `127.0.0.1` (Lite) | The address agent-api listens on inside the Lite container. agent-api is not a front door: reach it at the gateway's `/agent/*`. |
