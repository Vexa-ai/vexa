...
| `RECORD_VIDEO` | `false` | Whether a recording also captures the meeting **video** (the screen). Default records **audio only** — one ffmpeg per bot captures the meeting audio to live **HLS** (chunked fMP4/CMAF), no encode cost. `true` also captures the display. A `POST /bots` `record_video` overrides it per meeting. Playback is native `<video>` HLS in Safari/iOS + hls.js elsewhere. |
| `RECORD_VIDEO_CODEC` | `h264` | Video codec (when `RECORD_VIDEO=true`). `h264` (default — universal, native Safari/iOS). `hevc` (~40% smaller, Safari-native + NVENC, but Firefox support is spotty). `vp9` (royalty-free, Chrome/Firefox via hls.js but **not** native Safari, and CPU-heavy — no NVENC VP9). `av1` (smallest; Safari needs A17/M3+ hardware, software encode is slow). Composes with `VIDEO_HWACCEL`. |
| `RECORD_AUDIO_CODEC` | `aac` | Audio codec (applies to audio-only recordings too). `aac` (default — universal, native Safari/iOS HLS). `opus` (higher quality, Chrome/Firefox via hls.js but **not** Safari — Safari decodes no Opus in any HLS/streaming pipeline). |
| `VIDEO_HWACCEL` | `none` | Hardware accel for the video encode. `none` → software (cheap on libx264, expensive on libvpx-vp9/libsvtav1). `vaapi` → Intel/AMD VA-API (needs `/dev/dri` passthrough). `nvenc` → NVIDIA NVENC (needs the nvidia runtime). Composes with `RECORD_VIDEO_CODEC` — an impossible combo (e.g. `nvenc` + `vp9`) **fails the spawn** with a named error rather than silently. |
| `VIDEO_RESOLUTION` | `1920x1080` | Capture resolution as `WIDTHxHEIGHT`, e.g. `1280x800`. Sizes the bot's virtual display, the browser window, and the ffmpeg `x11grab` region together; a lower resolution cuts encode CPU roughly with the pixel count. |
| `RECORD_SEGMENT_SECONDS` | `15` | HLS segment length in seconds — the recorder ffmpeg's `-hls_time` and keyframe cadence, so each segment is independently decodable. |
| `CHROME_NO_SANDBOX` | (unset) | Escape hatch for the bot's browser sandbox. By default the bot runs **non-root with Chromium's namespace sandbox** — properly sandboxed, and no *"unsupported command-line flag"* banner in recordings. That requires the container to allow user namespaces, so bot containers are launched with `seccomp` unconfined (the k8s/Docker backends do this automatically; the **Lite** container needs `security_opt: [seccomp=unconfined]` in your compose — see [Deploy Lite](/deployment-lite)). Set `1` to keep the legacy path — run the bot as **root with `--no-sandbox`** — on hosts that can't relax seccomp. |
| `ENABLE_COMBINED_RECORDING` | `false` | Enable the **combined download** — one downloadable mp4 remuxed from the recording's HLS. Off ⇒ the player hides the download control. On ⇒ the player shows a download that builds **on-demand** the first time it's requested ("Preparing download…" → a link), or ahead of time with `AUTO_COMBINED_RECORDING`. |
| `AUTO_COMBINED_RECORDING` | `false` | When `ENABLE_COMBINED_RECORDING` is on, also build the combined download **automatically** the moment a recording completes, so it's ready without the first request paying the remux latency. |
| `RECORDING_COMBINED_AUDIO_CODEC` | (empty = copy) | Audio codec for the combined download. **Empty (default) stream-copies** the recording's audio into the mp4 — zero transcode, near-instant, and the download matches the recording (video is always copied too). Set it to an **ffmpeg encoder name to TRANSCODE** the audio: `aac` (universal, incl. QuickTime), `libopus` (smaller, but not QuickTime/Safari). Transcoding re-encodes the audio, so only set this when you specifically want the download in a different codec than the recording. |
