...
| `GUARD_WS_ENABLED` | `false` | Opt-in `/ws` connect guard. Runs the same library checks as the HTTP layer (ban store, IP lists, per-IP rate limit with its own `/ws` budget) and shares the Redis-backed ban store with it, so a ban set on either path holds on both. Only the `/ws` rate window is in-process: under `uvicorn --workers N>1` each worker keeps its own WS rate window. |
...
<Note>
With `GUARD_ENABLE_REDIS=false` AND `uvicorn --workers N>1`, the HTTP rate-limit buckets and auto-bans are also per-process: the effective limit becomes `N × GUARD_RATE_LIMIT_RPM` and bans do not propagate across workers (the `/ws` rate window is per-process regardless of this setting, and WS bans live in the same ban store as HTTP bans, so without Redis they are per-process too). The same applies across multiple gateway **replicas** without Redis — each is its own process.
