...
| `GUARD_IP_WHITELIST` | — | CSV of IPs or CIDR ranges that bypass every guard check (e.g. `10.0.0.0/8`). Validated at boot: an invalid entry refuses to start with an error naming the var and the entry. |
| `GUARD_IP_BLACKLIST` | — | CSV of IPs or CIDR ranges rejected on the first request (`403`), e.g. `203.0.113.5,198.51.100.0/24`. Validated at boot like the whitelist. |
| `GUARD_BLOCKED_COUNTRIES` | — | CSV of ISO country codes to block (geo, opt-in). |
| `GUARD_BLOCK_CLOUD_PROVIDERS` | — | CSV of cloud providers to block by IP range, opt-in. Accepted names: `AWS`, `Azure`, `DigitalOcean`, `GCP`, `Linode`, `Vultr`. An entry that is not an exact match refuses to start with an error naming the var, the entry, and the accepted spellings. With `GUARD_BLOCK_CLOUD_PROVIDERS_STRICT=true`, the match is case-insensitive and normalizes to the canonical spelling instead. A provider may be suffixed `:!<region>` (lowercase, or `GLOBAL`) to carve out one region exception. |
| `GUARD_BLOCK_CLOUD_PROVIDERS_STRICT` | `false` | Opt-in case-insensitivity for `GUARD_BLOCK_CLOUD_PROVIDERS`: normalizes provider case (`aws` -> `AWS`) instead of refusing to start. Off by default: an unrecognized or wrong-case name refuses to start either way, STRICT only decides whether case is forgiven. |
| `GUARD_TRUSTED_PROXIES` | — | CSV of trusted proxy IPs or CIDR ranges whose `X-Forwarded-For` is honored for client-IP resolution (e.g. `172.16.0.0/12`). Validated at boot. **Set this when behind a reverse proxy.** |
| `GUARD_TRUST_X_FORWARDED_PROTO` | `false` | Trust `X-Forwarded-Proto` for scheme enforcement. |
| `GUARD_REDIS_PREFIX` | `vexa:guard:` | Redis key namespace (avoids colliding with Vexa's own keys). guard-core 4.0.0 hashes the endpoint-path segment of the rate-limit key so it no longer appears in Redis logs/monitors verbatim; existing per-IP rate-limit counters reset once on the upgrade. |
| `GUARD_WS_ENABLED` | `false` | Opt-in `/ws` connect guard, run directly by fastapi-guard's own `guard_websocket`. Requires `GUARD_ENABLED=true` (boot refuses otherwise, naming both vars). Runs the same checks as the HTTP layer: ban store, IP lists, and a per-IP rate limit with its own `/ws` budget (an isolated `endpoint_path="ws"`), auto-banning through the same knob pair below. The ban store and the rate-limit window are both Redis-shared with the HTTP layer when `GUARD_ENABLE_REDIS=true`, per-process otherwise. Auto-ban violation counts are kept PER PATH: an IP's HTTP and WS over-limit events do not add up toward one threshold, though the resulting ban, once either path triggers it, is shared. **Breaking on upgrade:** a deploy that previously ran `GUARD_ENABLED=false` with `GUARD_WS_ENABLED=true` (a working WS-only setup under the old hand-rolled guard) now refuses to boot with a `ConfigError` naming both vars - set `GUARD_ENABLED=true` (or turn the WS guard off) before upgrading. |
| `GUARD_AUTO_BAN_RATE_LIMIT` | `true` | Auto-ban an IP that keeps exceeding the rate limit (ban reason `rate_limit_exceeded`) after `GUARD_AUTO_BAN_THRESHOLD` over-limit events, for `GUARD_AUTO_BAN_DURATION` seconds. One knob for both the HTTP pipeline and the `/ws` connect guard; needs the ban store (on by default). |

<Note>
With `GUARD_ENABLE_REDIS=false` AND `uvicorn --workers N>1`, the HTTP rate-limit buckets and auto-bans are also per-process: the effective limit becomes `N × GUARD_RATE_LIMIT_RPM` and bans do not propagate across workers (the `/ws` rate window now follows this same setting too, so without Redis it is per-process just like the HTTP window, and WS bans live in the same ban store as HTTP bans, so without Redis both are per-process). The same applies across multiple gateway **replicas** without Redis - each is its own process. On a Redis outage the gateway sets guard's `redis_fail_open=true`, so both windows degrade to their in-memory per-process fallback (one warning per process) rather than skipping the rate limit or failing closed; the same `N ×` ceiling applies until Redis is back.
...
The guard fails open (`fail_secure=false`): a guard-check bug returns the request to the app rather
than taking the gateway down, and a Redis outage keeps rate limiting on the per-process window
(`redis_fail_open=true`, see the Note above) while ban lookups answer "not banned" until Redis is back. Request-body WAF scanning is intentionally off — the
