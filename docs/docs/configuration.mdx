...
| `TRANSCRIPTION_REQUEST_TIMEOUT_MS` | unset (`30000` ms client default) | Optional per-request STT timeout, integer `1000`–`120000` milliseconds. Set directly in meeting-api's environment using a Compose service override or Helm `meetingApi.extraEnv`. Unset keeps existing behavior and retry settings. |
...
For Compose, add `TRANSCRIPTION_REQUEST_TIMEOUT_MS: "45000"` under
`services.meeting-api.environment` in an override file and recreate meeting-api.
Adding this variable to `.env` alone does not forward it to the service.
