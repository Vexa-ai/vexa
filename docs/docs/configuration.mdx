...
| `BOT_CAPTURE_FAULT_MAX_MS` | `3 x` the silence window, clamped to `60000`–`900000` | Upper bound on the **deaf-capture hold**. When the page reports connected remote streams but no audio frame arrives for a whole silence window, the bot assumes its own capture broke and suppresses `completed(left_alone)` rather than walking out of a live meeting. This caps how long it may do that: past the bound it leaves anyway. `0` disables the hold entirely. Forwarded to spawned bots. |
