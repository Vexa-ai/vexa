...
| `RECORDING_VIDEO_ENABLED` | `false` | Capture **video** as well as audio on a recording spawn. Off by default: video is roughly 20-50x the size of the audio and, like audio, is kept until you add a retention rule (below). The bot grabs its own X display with ffmpeg. |
| `RECORDING_PRUNE_CHUNKS` | `true` | Once a recording is complete and its assembled master is verified on storage, delete the 15-second chunks the master was built from. Halves the stored size of every recording anyone plays. A recording still in progress is never pruned. Set `false` to keep both copies. |
...
### Retention — nothing expires until you say so

Recordings are kept indefinitely. There is no TTL, no sweeper and no delete path in Vexa: deleting a
customer's meeting on a schedule is the kind of job you only want to run when someone has explicitly
asked for it.

A recording is stored as 15-second chunks while it is being made — so a bot killed mid-meeting
still leaves every finished part durable — and those chunks are assembled into one master file the
first time anyone plays it back. Once the recording is complete and that master is verified, the
chunks are deleted (`RECORDING_PRUNE_CHUNKS`, default on): the master holds the same bytes, so
keeping both would double the stored size of every recording you ever watch. A one-hour recording
ends up as one object rather than ~240.

What Vexa does instead is **tag every stored object with its media type** (`media=audio` /
`media=video`), so retention becomes a bucket lifecycle rule your object store enforces:

```bash
# keep audio forever, expire video after 90 days
mc ilm rule add --tags "media=video" --expire-days 90 myminio/vexa
mc ilm rule ls myminio/vexa
```

The tag is what makes this expressible at all: the media type sits in the *middle* of an object key
(`recordings/{user}/{recording}/{session}/{media_type}/…`) and lifecycle prefix filters only match a
literal prefix, so no prefix rule can select video without also selecting audio.

<Note>
Keeping meeting **video** indefinitely may carry privacy or records-retention obligations depending
on your jurisdiction and what participants were told. Decide the policy before the archive is large.
</Note>
