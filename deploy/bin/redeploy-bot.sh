...
# vexaai/vexa-bot:dev). It is COMPILED (TS → dist + an esbuild browser bundle), so unlike the Python
...
cd "$ROOT"
echo "▶ rebuilding vexaai/vexa-bot:dev from $(git log --oneline -1) …"
DOCKER_BUILDKIT=1 docker build -f core/meetings/services/bot/Dockerfile -t vexaai/vexa-bot:dev .
echo "✓ vexaai/vexa-bot:dev rebuilt — the next spawned bot picks it up (per-meeting; no container restart)."
