...
async function startCaptureForTab(tabId: number, url: string, meetingRef?: MeetingRef): Promise<void> {
  ...
  const ingestUrl: string = cfg.ingestUrl || 'ws://localhost:9099/ingest';
  ...
  console.log('[fixture-capture] connecting', fullUrl, 'platform=', meeting.platform, 'tab=', tabId);
    ...
    try {
      ...
      if (msg.type === 'ready') {
        ...
        if (state.platform === 'note') {
          ...
          // Zoom/Teams: remote audio is the MIXED tab-audio track via tabCapture.
          // The document_start WebRTC hook mirrors nothing for these clients —
          // Zoom Web decodes audio in WASM (0 RTCPeerConnection audio receivers),
          // so the only remote audio is the tab's output. The in-page mic ("You")
          // is captured separately. (Verified: 4 PCs, 0 audio receivers.)
          // Only attach mixed tab audio (999) if we already hold a stream id.
          // Content-script AUTO_START has none (no user gesture) — calling
          // startTabAudio() with a null id just fails and wedges the offscreen.
          // The panel's Start mints the id and re-attaches via the START handler.
          if ((state.platform === 'zoom' || state.platform === 'teams') && tabStreamId) {
            startTabAudio();
          }
          // Media tabs (YouTube): no content script, no WebRTC hook — mixed
          // tabCapture IS the remote audio; the local mic rides the offscreen path.
          if (isMediaTab) {
            startTabAudio();
            ensureOffscreen()
              .then(() => chrome.runtime.sendMessage({ type: 'NOTE_CAPTURE_START' }))
...
async function stopCapture(): Promise<void> {
  // Fixtures-only: Stop just tears down capture + closes the WS (the recorder
  // finalizes the fixture on close). No gateway/dashboard session-end call.
  if (state.platform === 'note') {
...
async function shipTelemetry(reason: string): Promise<void> {
  try {
    ...
    const ingest: string = cfg.ingestUrl || 'ws://localhost:9099/ingest';
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  switch (msg.type) {
    case 'START':
      console.log('[fixture-capture] START received, streamId=', msg.streamId, 'state=', state.status);
      tabStreamId = msg.streamId || null; // tab audio stream id minted in the panel (Zoom/Teams)
      // If a session is already live (content-script AUTO_START wins the race on
      // a reloaded tab) and we now hold a tab-audio stream id, attach the mixed
      // remote audio (999) mid-session instead of early-returning — otherwise
      // 999 never joins and we only ever get the mic + attribution.
      if (state.status === 'capturing' && tabStreamId && (state.platform === 'zoom' || state.platform === 'teams')) {
        startTabAudio();
      } else {
        startCaptureActiveTab();
      }
    ...
    case 'diag':
      if (msg.diag) {
        pageDiags[`${msg.diag.top ? 'top' : 'sub'}:${msg.diag.frame || '?'}`] = msg.diag;
        // Forward to the recorder so it lands in a file I can read. Background's
        // WS is NOT subject to page CSP. Sent as a typed text frame the recorder
        // special-cases (it is NOT a capture.v1 MeetingEvent).
        if (ws && ws.readyState === WebSocket.OPEN) {
          try { ws.send(JSON.stringify({ type: 'diag', ts: Date.now(), diag: msg.diag })); } catch { /* ignore */ }
        }
      }
      break;
    case 'speaker_activity':
      ...
      break;
    case 'chat-message':
      // capture.v1 `chat` event — sender + text, captured from the meeting's
      // chat panel. Unlike audio hints it is fine while paused (it's not audio).
...
// Fixtures-only: the action uses a popup (manifest action.default_popup), so
// there is NO side panel and NO action.onClicked — clicking the icon opens the
// popup, which mints the tabCapture stream id and sends START. (The product
// extension used a side panel here; removed to keep this self-contained.)
