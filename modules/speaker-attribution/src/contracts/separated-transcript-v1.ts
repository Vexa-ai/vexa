...
export interface SeparatedTranscriptSink {
  ...
  /** Optional LIVE PARTIAL (LocalAgreement-2 draft) for seg.speakerKey — replaces
   *  the prior draft under that key; empty text clears it. Confirmed-only consumers
   *  (recorder, batch attribution) omit it, so the channel is additive. */
