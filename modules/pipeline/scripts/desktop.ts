...
ingest.on('connection', async (ws, req) => {
  ...
  // ── gmeet GLOW lane — capture binds the glow name onto the audio (capture.v1
  //   speakerName); this per-speaker pipeline transcribes PER NAME → transcript.v1.
  //   The remote channels (0/1/2 = an anonymous rotating pool) are routed by the
  //   BOUND NAME, never the channel index — no diarizer, no post-hoc attribution.
  const gmeetPipe = txClient ? createGmeetPipeline({
    transcribe,
    ...
    sink: {
      segment: (t) => {
        const seg: Seg = { segment_id: `${metaKey}:${t.speakerKey}:${Math.round(t.start * 1000)}`, speaker: t.speaker, text: t.text, start: t.start, absolute_start_time: new Date(t.start * 1000).toISOString(), completed: true };
        broadcast(metaKey, t.speaker, [seg], []);
      },
      // live forming tail → pending (empty text clears the speaker's draft)
      draft: (t) => {
        const pend: Seg[] = t.text.trim() ? [{ segment_id: `${metaKey}:${t.speakerKey}:pending`, speaker: t.speaker, text: t.text, start: t.start, absolute_start_time: new Date(t.start * 1000).toISOString(), completed: false }] : [];
        broadcast(metaKey, t.speaker, [], pend);
      },
  ...
  ws.on('message', (data: any, isBinary: boolean) => {
    try {
      ...
      // active-speaker hints name the MIXED 999 clusters (zoom/teams) post-hoc. gmeet
      // no longer needs them here — capture binds the glow name onto the audio itself.
      ...
      // speaker-joined is roster only — names ride on the audio (gmeet) or the hints (mixed).
