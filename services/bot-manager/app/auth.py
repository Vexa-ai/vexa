...
async def get_api_key(api_key: str = Security(API_KEY_HEADER),
    ...
    if not api_key:
        logger.debug("API token missing from header")
        raise HTTPException(
    ...
    logger.debug(f"Received API key: {api_key[:5]}...")
    ...
    if not token_user:
        logger.debug(f"Invalid API token provided: {api_key[:5]}...")
    ...
    if not isinstance(user_obj, User):
         logger.debug(f"get_api_key did not retrieve a valid User object: {type(user_obj)}")
         raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Authentication data error")

    logger.debug(f"API key validated for user {user_obj.id}. Returning key string and user object.")
...
async def get_user_and_token(token_user_tuple: tuple[str, User] = Depends(get_api_key)) -> tuple[str, User]:
    ...
    if not isinstance(token_user_tuple, tuple) or len(token_user_tuple) != 2:
        logger.debug(f"get_user_and_token received invalid input: {type(token_user_tuple)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Authentication processing error")
    logger.debug(f"Providing API key and User object for user {token_user_tuple[1].id}")
...
async def get_current_user(user_and_token: tuple[str, User] = Depends(get_user_and_token)) -> User:
    ...
    logger.debug(f"get_current_user providing User object for user {user.id}")
