...
class DockerClient:
    ...
    def _count_running_bots_for_user(self, user_id: str) -> int:
        ...
        try:
            ...
            logger.debug(f"Error counting running containers for user {user_id}: {e}")
    ...
    def create_bot_container(self, user_id: str, meeting_id: str, meeting_url: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if existing_container:
                logger.debug(f"Container {container_name} already exists with status: {existing_container.status}")
                
                # Start container if it's not running
                if existing_container.status != "running":
                    existing_container.start()
                    logger.debug(f"Started existing container {container_name}")
        ...
        try:
            ...
            if not user:
                 ...
                 logger.debug(f"User with ID {user_id} not found and could not be created.")
            ...
            logger.debug(f"Checking bot limit for user {user_id}: Found {current_bot_count} running bots, limit is {user_limit}") # Added logging
            
            if not hasattr(user, 'max_concurrent_bots') or user_limit is None: # Check variable
                 logger.debug(f"User {user_id} is missing the max_concurrent_bots attribute or it's None.")
            ...
            if current_bot_count >= user_limit: # Check variable
                logger.debug(f"User {user_id} reached bot limit ({user_limit}). Cannot create new bot.") # Use variable in log
                raise HTTPException(
                    status_code=403, # Forbidden
                    detail=f"User has reached the maximum concurrent bot limit ({user_limit})." # Use variable in detail
                )
            logger.debug(f"User {user_id} is under bot limit ({current_bot_count}/{user_limit}). Proceeding...") # Use variable in log

        except HTTPException as http_exc:
             raise http_exc # Re-raise HTTP exceptions directly
        except Exception as e:
             # Catch potential DB or Docker errors during the check
             logger.debug(f"Error during bot limit check for user {user_id}: {e}")
        ...
        logger.debug(f"Creating bot container for meeting URL: {meeting_url} for user {user_id}")
        ...
        try:
            ...
            logger.debug(f"Created container {container_name} with label vexa.user_id={user_id}")
            return {"status": "created", "container_name": container_name}
        except Exception as e:
            logger.debug(f"Error creating container: {e}")
            raise
    ...
    def delete_bot_container(self, user_id: str, meeting_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            if meeting_id:
                ...
                container.stop()
                container.remove()
                logger.debug(f"Deleted container {container_name}")
                ...
                for container in containers:
                    container.stop()
                    container.remove()
                    logger.debug(f"Deleted container {container.name}")
                return {"status": "deleted", "count": len(containers)}
        except docker.errors.NotFound:
            logger.debug(f"Container not found for user {user_id}")
            return {"status": "not_found"}
        except Exception as e:
            logger.debug(f"Error deleting container: {e}")
            raise
