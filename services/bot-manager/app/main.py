...
from shared_models.database import async_session_local, get_db, init_db
...
async def update_meeting_status(
    ...
    try:
        ...
        logger.debug(f"Invalid meeting status '{meeting.status}' for meeting {meeting.id}, normalizing to 'failed'")
    ...
    if not is_valid_status_transition(current_status, new_status):
        logger.debug(f"Invalid status transition from '{current_status.value}' to '{new_status.value}' for meeting {meeting.id}")
        logger.debug(f"[DEBUG] Invalid transition: current='{current_status.value}', requested='{new_status.value}', meeting_id={meeting.id}")
        return False
    ...
    logger.debug(f"Meeting {meeting.id} status updated from '{old_status}' to '{new_status.value}'")
    return True
...
async def publish_meeting_status_change(meeting_id: int, new_status: str, redis_client: Optional[aioredis.Redis], platform: str, native_meeting_id: str, user_id: int):
    ...
    if not redis_client:
        logger.debug("Redis client not available for publishing meeting status change")
        return
    try:
        payload = {
            "type": "meeting.status",
        ...
        logger.debug(f"Published meeting status change to '{channel}': {new_status}")
    except Exception as e:
        logger.debug(f"Failed to publish meeting status change for meeting {meeting_id}: {e}")
...
async def schedule_status_webhook_task(
    ...
    logger.debug(f"Scheduled status webhook task for meeting {meeting.id} status change: {old_status} -> {new_status}")
...
async def startup_event():
    ...
    logger.debug("Starting up Bot Manager...")
    # await init_db() # Removed - Admin API should handle this
    # await init_redis() # Removed redis init if not used elsewhere
    try:
        get_socket_session()
    except Exception as e:
        logger.debug(f"Failed to initialize Docker client on startup: {e}", exc_info=True)
    ...
    try:
        ...
        logger.debug(f"Failed to connect to Redis on startup: {e}", exc_info=True)
        redis_client = None # Ensure client is None if connection fails
    # --------------------------------------

    logger.debug("Database, Docker Client (attempted), and Redis Client (attempted) initialized.")
    
    # Start reconciliation scheduler
    logger.debug("[Startup] Starting reconciliation scheduler...")
    asyncio.create_task(start_reconciliation_scheduler())
    logger.debug("[Startup] Reconciliation scheduler started")

@app.on_event("shutdown")
async def shutdown_event():
    global redis_client # <-- Add global reference
    logger.debug("Shutting down Bot Manager...")
    ...
    if redis_client:
        ...
        try:
            ...
            logger.debug(f"Error closing Redis connection: {e}", exc_info=True)
    # ---------------------------------
...
async def _delayed_container_stop(container_id: str, meeting_id: int, delay_seconds: int = 30):
    ...
    """
    logger.debug(f"[Delayed Stop] Task started for container {container_id} (meeting {meeting_id}). Waiting {delay_seconds}s before stopping.")
    await asyncio.sleep(delay_seconds)
    logger.debug(f"[Delayed Stop] Delay finished for {container_id}. Attempting synchronous stop...")
    try:
        ...
        logger.debug(f"[Delayed Stop] Successfully stopped container {container_id}.")
    except Exception as e:
        logger.debug(f"[Delayed Stop] Error stopping container {container_id}: {e}", exc_info=True)
    ...
    try:
        ...
        logger.debug(f"[Delayed Stop] Waiting {grace_period}s grace period for pending callbacks before finalizing meeting {meeting_id}...")
        ...
        async with async_session_local() as db:
            ...
            if not meeting:
                logger.debug(f"[Delayed Stop] Meeting {meeting_id} not found in DB. Cannot finalize.")
                return
            ...
            if meeting.status not in terminal_states:
                logger.debug(f"[Delayed Stop] Meeting {meeting_id} still in non-terminal state '{meeting.status}' after container stop. Finalizing to COMPLETED (callback missed).")
                ...
                if success:
                    ...
                    logger.debug(f"[Delayed Stop] Meeting {meeting_id} finalized to COMPLETED and post-meeting tasks scheduled.")
                else:
                    logger.debug(f"[Delayed Stop] Failed to finalize meeting {meeting_id} to COMPLETED.")
            else:
                logger.debug(f"[Delayed Stop] Meeting {meeting_id} already in terminal state '{meeting.status}'. No finalization needed.")
    except Exception as e:
        logger.debug(f"[Delayed Stop] Error during safety finalizer for meeting {meeting_id}: {e}", exc_info=True)
# --- ------------------------ ---
...
async def request_bot(
    ...
    logger.debug(f"Received bot request for platform '{req.platform.value}' with native ID '{req.native_meeting_id}' from user {current_user.id}")
    ...
    if not constructed_url:
        logger.debug(f"Invalid meeting URL for platform {req.platform.value} and ID {native_meeting_id}. Rejecting request.")
        raise HTTPException(
    ...
    if existing_meeting:
        logger.debug(f"Found existing meeting record {existing_meeting.id} with status '{existing_meeting.status}' for user {current_user.id}, platform '{req.platform.value}', native ID '{native_meeting_id}'.")
    ...
    if user_limit > 0:
        ...
        if active_count >= user_limit:
            logger.debug(f"User {current_user.id} reached concurrent bot limit {active_count}/{user_limit}. Rejecting new launch.")
            raise HTTPException(
    ...
    if existing_meeting is None:
        logger.debug(f"No active/valid existing meeting found for user {current_user.id}, platform '{req.platform.value}', native ID '{native_meeting_id}'. Proceeding to create a new meeting record.")
        ...
        logger.debug(f"Created new meeting record with ID: {meeting_id_for_bot}")
        ...
        try:
            ...
            logger.debug(f"Published initial meeting.status 'requested' for meeting {meeting_id_for_bot}")
        except Exception as _pub_err:
            logger.debug(f"Failed to publish initial 'requested' status for meeting {meeting_id_for_bot}: {_pub_err}")
        ...
        logger.debug(f"Logic error: Should have raised 409 for existing meeting {existing_meeting.id}, but proceeding.")
    ...
    if 'new_meeting' in locals() and new_meeting is not None:
        ...
        logger.debug(f"Critical logic error: Reached container start without a definitive meeting object for platform '{req.platform.value}', native ID '{native_meeting_id}'.")
    ...
    if invalid_fields:
        logger.debug(f"Preflight validation failed. Invalid fields: {invalid_fields}")
        try:
    ...
    connection_id = None
    try:
        logger.debug(f"Attempting to start bot container for meeting {meeting_id} (native: {native_meeting_id})...")
        ...
        logger.debug(f"Call to start_bot_container completed. Container ID: {container_id}, Connection ID: {connection_id}")
        ...
        if not container_id or not connection_id:
            ...
            logger.debug(f"{error_msg} for meeting {meeting_id}")
        ...
        logger.debug(f"Scheduled background task to record session start for meeting {meeting_id}, session {connection_id}")
        ...
        logger.debug(f"Setting container ID {container_id} for meeting {meeting_id} (status remains 'requested' until bot confirms startup)")
        ...
        logger.debug(f"Successfully set container ID for meeting {meeting_id}. Status remains 'requested' until bot startup callback.")

        logger.debug(f"Successfully started bot container {container_id} for meeting {meeting_id}")
        return MeetingResponse.model_validate(current_meeting_for_bot_launch)

    except HTTPException as http_exc:
        logger.debug(f"HTTPException occurred during bot startup for meeting {meeting_id}: {http_exc.status_code} - {http_exc.detail}")
        try:
            ...
            if meeting_to_update and meeting_to_update.status not in [MeetingStatus.FAILED.value, MeetingStatus.COMPLETED.value]: 
                 logger.debug(f"Updating meeting {meeting_id} status to 'failed' due to HTTPException {http_exc.status_code}.")
                ...
                logger.debug(f"Could not find meeting {meeting_id} to update status to error after HTTPException.")
        except Exception as db_err:
             logger.debug(f"Failed to update meeting {meeting_id} status to error after HTTPException: {db_err}")
        raise http_exc

    except Exception as e:
        logger.debug(f"Unexpected exception occurred during bot startup process for meeting {meeting_id} (after DB creation): {e}", exc_info=True)
        try:
            meeting_to_update = await db.get(Meeting, meeting_id) # Re-fetch
            if meeting_to_update and meeting_to_update.status not in [MeetingStatus.FAILED.value, MeetingStatus.COMPLETED.value]:
                 logger.debug(f"Updating meeting {meeting_id} status to 'failed' due to unexpected exception.")
                 meeting_to_update.status = MeetingStatus.FAILED.value
                 if container_id:
                     meeting_to_update.bot_container_id = container_id
                 await db.commit()
                ...
                logger.debug(f"Could not find meeting {meeting_id} to update status to error after unexpected exception.")
        except Exception as db_err:
             logger.debug(f"Failed to update meeting {meeting_id} status to error after unexpected exception: {db_err}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_bot_config(
    ...
    logger.debug(f"User {current_user.id} requesting config update for {platform.value}/{native_meeting_id}: lang={req.language}, task={req.task}")
    ...
    if not active_meeting:
        logger.debug(f"No active meeting found for user {current_user.id}, {platform.value}/{native_meeting_id} to reconfigure.")
    ...
    logger.debug(f"[DEBUG] Found active meeting record with internal ID: {internal_meeting_id}")

    # 2. Construct and Publish command (meeting-based addressing only)
    if not redis_client:
        logger.debug("Redis client not available. Cannot publish reconfigure command.")
        raise HTTPException(
    ...
    try:
        ...
        logger.debug(f"Publishing command to channel '{channel}': {payload_str}")
        await redis_client.publish(channel, payload_str)
        logger.debug(f"Successfully published reconfigure command for meeting {internal_meeting_id}.")
    except Exception as e:
        logger.debug(f"Failed to publish reconfigure command to Redis channel {channel}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def stop_bot(
    ...
    logger.debug(f"Received stop request for {platform_value}/{native_meeting_id} from user {current_user.id}")# 1. Find all meetings matching the criteria
    ...
    if not all_meetings:
        logger.debug(f"Stop request: No meeting found for {platform_value}/{native_meeting_id} for user {current_user.id}")
    ...
    if not non_terminal_meetings:
        ...
        logger.debug(f"Stop request: Meeting {meeting.id} already in terminal state '{meeting.status}'. Returning 202 idempotently.")
    ...
    for meeting in non_terminal_meetings:
        ...
        if not meeting.bot_container_id:
            logger.debug(f"Stop request: Meeting {meeting.id} has no container ID (status: {meeting.status}). Finalizing immediately.")
            success = await update_meeting_status(
                meeting, 
                MeetingStatus.COMPLETED, 
                db,
            ...
            logger.debug(f"Scheduling post-meeting tasks for meeting {meeting.id} (no container case).")
            background_tasks.add_task(run_all_tasks, meeting.id)
            continue

        logger.debug(f"Found meeting {meeting.id} (status: {meeting.status}) with container {meeting.bot_container_id} for stop request.")
        ...
        if meeting.status in [MeetingStatus.REQUESTED.value, MeetingStatus.JOINING.value, MeetingStatus.AWAITING_ADMISSION.value] and (seconds_since_created is not None and seconds_since_created < 5):
            logger.debug(f"Stop request: Meeting {meeting.id} is pre-active and started {seconds_since_created:.2f}s ago. Finalizing immediately and stopping container.")
        ...
        if not redis_client:
            logger.debug("Redis client not available. Cannot send leave command.")
            ...
            try:
                ...
                logger.debug(f"Publishing leave command to Redis channel '{command_channel}': {payload}")
                await redis_client.publish(command_channel, payload)
                logger.debug(f"Successfully published leave command for meeting {meeting.id}.")
            except Exception as e:
                logger.debug(f"Failed to publish leave command to Redis channel {command_channel}: {e}", exc_info=True)
                # Log error but continue with delayed stop

        # 4. Schedule delayed container stop task
        logger.debug(f"Scheduling delayed stop task for container {meeting.bot_container_id} (meeting {meeting.id}).")
        ...
        if success:
            logger.debug(f"Stop request accepted for meeting {meeting.id}. Status updated from '{old_status}' to 'stopping'. Bot will transition to completed/failed via callback.")
        else:
            logger.debug(f"Stop request: Failed to update meeting {meeting.id} status to 'stopping' (invalid transition from '{old_status}'). Proceeding anyway.")
        ...
        logger.debug(f"Stop request for meeting {meeting.id} accepted. Leave command sent, delayed stop scheduled.")
...
async def get_user_bots_status(
    ...
    logger.debug(f"Fetching running bot status for user {user_id}")
    ...
    try:
        ...
        logger.debug(f"Error fetching bot status for user {user_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def bot_exit_callback(
    ...
    """
    logger.debug(f"Received bot exit callback: connection_id={payload.connection_id}, exit_code={payload.exit_code}, reason={payload.reason}")
    ...
    try:
        ...
        if not meeting_session:
            logger.debug(f"Bot exit callback: Could not find meeting session for connection_id {session_uid}. Cannot update meeting status.")
        ...
        logger.debug(f"Bot exit callback: Found meeting_id {meeting_id} for connection_id {session_uid}")

        # Now get the full meeting object
        meeting = await db.get(Meeting, meeting_id)
        if not meeting:
            logger.debug(f"Bot exit callback: Found session but could not find meeting {meeting_id} itself.")
        ...
        if exit_code == 0:
            ...
            if success:
                ...
                logger.debug(f"Bot exit callback: Meeting {meeting_id} status updated to 'completed'.")
            else:
                logger.debug(f"Bot exit callback: Failed to update meeting {meeting_id} status to 'completed'")
            ...
            if success:
                ...
                logger.debug(f"Bot exit callback: Meeting {meeting_id} status updated to 'failed' due to exit_code {exit_code}.")
            else:
                logger.debug(f"Bot exit callback: Failed to update meeting {meeting_id} status to 'failed'")
                return {"status": "error", "detail": "Failed to update meeting status"}
            ...
            if payload.error_details or payload.platform_specific_error:
                ...
                logger.debug(f"Bot exit callback: Stored error details in meeting {meeting_id} data: {error_data}")
        
        meeting.end_time = datetime.utcnow()
        await db.commit()
        await db.refresh(meeting)
        logger.debug(f"Bot exit callback: Meeting {meeting.id} successfully updated in DB.")
        ...
        logger.debug(f"Bot exit callback: Scheduling post-meeting tasks for meeting {meeting.id}.")
        background_tasks.add_task(run_all_tasks, meeting.id)
        ...
        if exit_code != 0 and meeting.bot_container_id:
            logger.debug(f"Bot exit callback: Scheduling delayed stop for container {meeting.bot_container_id} of failed meeting {meeting.id}.")
        ...
        logger.debug(f"Bot exit callback: An unexpected error occurred: {e}", exc_info=True)
...
async def bot_startup_callback(
    ...
    """
    logger.debug(f"Received bot startup callback: connection_id={payload.connection_id}, container_id={payload.container_id}")
    ...
    try:
        ...
        if not meeting_session:
            logger.debug(f"Bot startup callback: Could not find meeting session for connection_id {session_uid}. Cannot update meeting status.")
            return {"status": "error", "detail": "Meeting session not found"}

        meeting_id = meeting_session.meeting_id
        logger.debug(f"Bot startup callback: Found meeting_id {meeting_id} for connection_id {session_uid}")

        # Now get the full meeting object
        meeting = await db.get(Meeting, meeting_id)
        if not meeting:
            logger.debug(f"Bot startup callback: Found session but could not find meeting {meeting_id} itself.")
            return {"status": "error", "detail": f"Meeting {meeting_id} not found"}
        ...
        if meeting.data and isinstance(meeting.data, dict) and meeting.data.get("stop_requested"):
            logger.debug(f"Bot startup callback: stop_requested set for meeting {meeting_id}. Ignoring startup transition.")
        ...
        if meeting.status in [MeetingStatus.REQUESTED.value, MeetingStatus.JOINING.value, MeetingStatus.AWAITING_ADMISSION.value, MeetingStatus.FAILED.value]:
            ...
            if success:
                ...
                await db.commit()
                await db.refresh(meeting)
                logger.debug(f"Bot startup callback: Meeting {meeting_id} status updated from '{old_status}' to 'active' with container {container_id}.")
                # No manual transition writes here; update_meeting_status already recorded the transition
            else:
                logger.debug(f"Bot startup callback: Failed to update meeting {meeting_id} status to 'active'")
                return {"status": "error", "detail": "Failed to update meeting status"}
            ...
            await db.commit()
            await db.refresh(meeting)
            logger.debug(f"Bot startup callback: Meeting {meeting_id} already active, updated container ID to {container_id}.")
        else:
            logger.debug(f"Bot startup callback: Meeting {meeting_id} has unexpected status '{meeting.status}', not updating.")
        ...
        logger.debug(f"Bot startup callback: An unexpected error occurred: {e}", exc_info=True)
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def bot_joining_callback(
    ...
    """
    logger.debug(f"Received bot joining callback: connection_id={payload.connection_id}, container_id={payload.container_id}")
    ...
    try:
        ...
        if not meeting_session:
            logger.debug(f"Bot joining callback: Could not find meeting session for connection_id {session_uid}. Cannot update meeting status.")
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        if not meeting:
            logger.debug(f"Bot joining callback: Could not find meeting for session {meeting_session.meeting_id}. Cannot update meeting status.")
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        if meeting.data and isinstance(meeting.data, dict) and meeting.data.get("stop_requested"):
            logger.debug(f"Bot joining callback: stop_requested set for meeting {meeting.id}. Ignoring joining transition.")
            return {"status": "ignored", "detail": "stop requested"}
        ...
        if success:
            logger.debug(f"Bot joining callback: Successfully updated meeting {meeting.id} status to 'joining'")
        ...
        logger.debug(f"Bot joining callback: An unexpected error occurred: {e}", exc_info=True)
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def bot_awaiting_admission_callback(
    ...
    """
    logger.debug(f"Received bot awaiting admission callback: connection_id={payload.connection_id}, container_id={payload.container_id}")
    ...
    try:
        ...
        if not meeting_session:
            logger.debug(f"Bot awaiting admission callback: Could not find meeting session for connection_id {session_uid}. Cannot update meeting status.")
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Meeting session not found for connection_id: {session_uid}"
            )
        ...
        if not meeting:
            logger.debug(f"Bot awaiting admission callback: Could not find meeting for session {meeting_session.meeting_id}. Cannot update meeting status.")
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Meeting not found for session: {meeting_session.meeting_id}"
            )
        ...
        if meeting.data and isinstance(meeting.data, dict) and meeting.data.get("stop_requested"):
            logger.debug(f"Bot awaiting admission callback: stop_requested set for meeting {meeting.id}. Ignoring waiting room transition.")
            return {"status": "ignored", "detail": "stop requested"}
        ...
        if success:
            logger.debug(f"Bot awaiting admission callback: Successfully updated meeting {meeting.id} status to 'awaiting_admission'")
            # Publish status change to Redis
        ...
        logger.debug(f"Bot awaiting admission callback: An unexpected error occurred: {e}", exc_info=True)
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def bot_status_change_callback(
    ...
    """
    logger.debug(f"Received unified bot status change callback: connection_id={payload.connection_id}, status={payload.status.value}, reason={payload.reason}")
    ...
    try:
        ...
        if not meeting_session:
            logger.debug(f"Bot status change callback: Could not find meeting session for connection_id {session_uid}")
            return {"status": "error", "detail": f"Meeting session not found for connection_id: {session_uid}"}

        meeting_id = meeting_session.meeting_id
        logger.debug(f"Bot status change callback: Found meeting_id {meeting_id} for connection_id {session_uid}")

        # Get the full meeting object and refresh to ensure we have latest status
        meeting = await db.get(Meeting, meeting_id)
        if not meeting:
            logger.debug(f"Bot status change callback: Could not find meeting {meeting_id}")
            return {"status": "error", "detail": f"Meeting {meeting_id} not found"}
        
        # Refresh meeting to get latest status from database
        await db.refresh(meeting)
        logger.debug(f"[DEBUG] Bot status change callback: Meeting {meeting_id} current status='{meeting.status}', requested status='{new_status.value}'")
        ...
        if (meeting.data and isinstance(meeting.data, dict) and 
            ...
            logger.debug(f"Bot status change callback: stop_requested set for meeting {meeting.id}. Ignoring {new_status.value} transition.")
            return {"status": "ignored", "detail": "stop requested"}
        ...
        if new_status == MeetingStatus.COMPLETED:
            ...
            if meeting.status in [MeetingStatus.REQUESTED.value, MeetingStatus.JOINING.value, MeetingStatus.AWAITING_ADMISSION.value, MeetingStatus.FAILED.value]:
                ...
                await db.commit()
                await db.refresh(meeting)
                logger.debug(f"Bot status change callback: Meeting {meeting_id} already active, updated container ID to {payload.container_id}")
                return {"status": "container_updated", "meeting_id": meeting.id, "meeting_status": meeting.status}
            else:
                logger.debug(f"Bot status change callback: Meeting {meeting_id} has unexpected status '{meeting.status}', not updating to active")
                success = False
            ...
            if not success:
                logger.debug(f"Bot status change callback: Failed to update meeting {meeting_id} status to '{new_status.value}'")
                # #region agent log
                try:
                    with open('/home/dima/dev/.cursor/debug.log', 'a') as f:
                        import json
        ...
        logger.debug(f"Bot status change callback: An unexpected error occurred: {e}", exc_info=True)
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def reconcile_meetings_and_containers():
    ...
    """
    logger.debug("[Reconciliation] Starting reconciliation task...")
    ...
    try:
        async with async_session_local() as db:
            ...
            logger.debug(f"[Reconciliation] Found {len(non_terminal_meetings)} non-terminal meetings to check")
                ...
                if not container_exists:
                    ...
                    if is_likely_name:
                        ...
                        try:
                            ...
                            logger.debug(f"[Reconciliation] Error checking running bots for safety check: {e}")
                    ...
                    if success:
                        ...
                        logger.debug(f"[Reconciliation] Fixed zombie meeting {meeting.id}")
                    else:
                        logger.debug(
            ...
            try:
                ...
                logger.debug(f"[Reconciliation] Checking running bots for {len(user_ids)} users")
                ...
                for user_id in user_ids:
                    try:
                        ...
                        logger.debug(f"[Reconciliation] Error getting running bots for user {user_id}: {e}", exc_info=True)
            except Exception as e:
                logger.debug(f"[Reconciliation] Error listing running bots: {e}", exc_info=True)
            
            logger.debug(f"[Reconciliation] Found {len(all_running_bots)} running bots to check")
            ...
            async with async_session_local() as db2:
                for bot_info in all_running_bots:
                    ...
                    if not meeting:
                        ...
                        logger.debug(f"[Reconciliation] ORPHAN CONTAINER detected: Container {container_id} has meeting_id {meeting_id} but meeting doesn't exist. Killing container...")
                        try:
                            stop_bot_container(container_id)
                            orphan_containers_killed += 1
                            logger.debug(f"[Reconciliation] Killed orphan container {container_id}")
                        except Exception as e:
                            logger.debug(f"[Reconciliation] Failed to kill orphan container {container_id}: {e}")
                        continue
                    
                    # Check if meeting is in terminal state
                    terminal_states = [MeetingStatus.COMPLETED.value, MeetingStatus.FAILED.value]
                    if meeting.status in terminal_states:
                        logger.debug(f"[Reconciliation] ORPHAN CONTAINER detected: Container {container_id} is running but meeting {meeting_id} is {meeting.status}. Killing container...")
                        try:
                            stop_bot_container(container_id)
                            orphan_containers_killed += 1
                            logger.debug(f"[Reconciliation] Killed orphan container {container_id} for terminal meeting {meeting_id}")
                        except Exception as e:
                            logger.debug(f"[Reconciliation] Failed to kill orphan container {container_id}: {e}")
            
            logger.debug(f"[Reconciliation] Reconciliation complete: {zombie_meetings_fixed} zombie meetings fixed, {orphan_containers_killed} orphan containers killed")
    except Exception as e:
        logger.debug(f"[Reconciliation] Error during reconciliation: {e}", exc_info=True)
...
async def start_reconciliation_scheduler():
    ...
    while True:
        try:
            ...
            logger.debug(f"[Reconciliation Scheduler] Error: {e}", exc_info=True)
