...
async def init_redis():
    ...
    if redis_client is None:
        try:
            ...
            logger.debug(f"Could not connect to Redis: {e}", exc_info=True)
...
def get_redis_client():
    ...
    if redis_client is None:
        ...
        logger.debug("Redis client requested before initialization.")
...
def extract_platform_specific_id(platform: str, meeting_url: str) -> Optional[str]:
    ...
    try:
        if platform == "google_meet":
            ...
            # https://teams.live.com/meet/12345678901234 or with ?p=passcode
            match = re.search(r'(?:teams\.live\.com/meet/)(\d{10,15})', meeting_url)
            if match:
                return match.group(1)
        else:
            logger.debug(f"Platform '{platform}' URL parsing not implemented or pattern mismatch for URL: {meeting_url}")
        return None
    except Exception as e:
        logger.debug(f"Error extracting platform_specific_id for {platform}/{meeting_url}: {e}", exc_info=True)
        return None
...
async def acquire_lock(meeting_id: str) -> bool:
    ...
    global redis_client
    if not redis_client:
        logger.debug("Cannot acquire lock, Redis client not initialized")
        return False
    ...
    try:
        ...
        was_set = await redis_client.set(lock_key, "locked", nx=True, ex=LOCK_TTL_SECONDS)
        if was_set:
            logger.debug(f"Acquired lock: {lock_key} for {LOCK_TTL_SECONDS}s")
            return True
        else:
            # Check TTL of existing lock to provide more context
            ttl = await redis_client.ttl(lock_key)
            logger.debug(f"Failed to acquire lock (already held): {lock_key} (TTL: {ttl}s)")
            return False
    except Exception as e:
        logger.debug(f"Error acquiring Redis lock for {meeting_id}: {e}", exc_info=True)
        return False
...
async def release_lock(meeting_id: str):
    ...
    global redis_client
    if not redis_client:
        logger.debug("Cannot release lock/map, Redis client not initialized")
        return
    ...
    try:
        ...
        logger.debug(f"Attempted release for meeting_id {meeting_id}. Keys deleted: {deleted_count}")
        if deleted_count == 0:
            logger.debug(f"Neither lock key '{lock_key}' nor map key '{map_key}' found for deletion.")
        elif deleted_count == 1:
            # Check which one might still exist (though delete is idempotent)
            if await redis_client.exists(lock_key):
                logger.debug(f"Released map but lock key '{lock_key}' was not found.")
            elif await redis_client.exists(map_key):
                logger.debug(f"Released lock but map key '{map_key}' was not found.")
            else: # Should not happen if count is 1
                logger.debug(f"Released one key for {meeting_id}, but subsequent existence check found none.")
        else: # deleted_count == 2
            logger.debug(f"Successfully released lock '{lock_key}' and mapping '{map_key}'.")

    except Exception as e:
        logger.debug(f"Failed to release lock or mapping for {meeting_id}: {e}", exc_info=True)
...
async def store_container_mapping(meeting_id: str, container_id: str):
    ...
    global redis_client
    if not redis_client:
        logger.debug("Cannot store mapping, Redis client not initialized")
        return
    map_key = generate_container_mapping_key(meeting_id)
    try:
        await redis_client.set(map_key, container_id, ex=MAPPING_TTL_SECONDS)
        logger.debug(f"Stored container mapping: {map_key} -> {container_id} for {MAPPING_TTL_SECONDS}s")
    except Exception as e:
        logger.debug(f"Failed to store container mapping for {meeting_id}: {e}", exc_info=True)
...
async def get_container_id_for_meeting(meeting_id: str) -> Optional[str]:
    ...
    global redis_client
    if not redis_client:
        logger.debug("Cannot get mapping, Redis client not initialized")
        return None
    map_key = generate_container_mapping_key(meeting_id)
    try:
        container_id = await redis_client.get(map_key)
        if container_id:
            logger.debug(f"Retrieved container mapping: {map_key} -> {container_id}")
            ...
            logger.debug(f"No container mapping found for key: {map_key}")
            return None
    except Exception as e:
        logger.debug(f"Failed to get container mapping for {meeting_id}: {e}", exc_info=True)
        return None 