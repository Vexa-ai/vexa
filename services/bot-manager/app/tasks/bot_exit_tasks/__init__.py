...
async def run_all_tasks(meeting_id: int):
    ...
    """
    logger.debug(f"Starting to run all post-meeting tasks for meeting_id: {meeting_id}")
    ...
    async with async_session_local() as db:
        try:
            ...
            if not meeting:
                logger.debug(f"Could not find meeting with ID {meeting_id} to run post-meeting tasks.")
                return
                ...
                if filename.endswith('.py') and filename != '__init__.py':
                    ...
                    try:
                        ...
                        if hasattr(module, 'run') and inspect.iscoroutinefunction(module.run):
                            logger.debug(f"Found task in '{module_name}'. Executing for meeting {meeting_id}...")
                            try:
                                # All tasks are now async and receive the same arguments
                                await module.run(meeting, db)
                                logger.debug(f"Successfully executed task in '{module_name}' for meeting {meeting_id}.")
                            except Exception as e:
                                logger.debug(f"Error executing task in '{module_name}' for meeting {meeting_id}: {e}", exc_info=True)
                        else:
                            logger.debug(f"Module '{module_name}' does not have a valid async 'run' function.")
                    
                    except ImportError as e:
                        logger.debug(f"Failed to import task module '{module_name}': {e}", exc_info=True)
            
            await db.commit()
            logger.debug(f"All post-meeting tasks run and changes committed for meeting_id: {meeting_id}")

        except Exception as e:
            logger.debug(f"An error occurred in the task runner for meeting_id {meeting_id}: {e}", exc_info=True)
            await db.rollback() 