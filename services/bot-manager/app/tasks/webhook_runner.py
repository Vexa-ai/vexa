...
async def run_status_webhook_task(meeting_id: int, status_change_info: dict = None):
    ...
    """
    logger.debug(f"Starting webhook task runner for meeting {meeting_id}")
    ...
    async with async_session_local() as db:
        try:
            ...
            if not meeting:
                logger.debug(f"Could not find meeting with ID {meeting_id} for webhook task")
                return
            ...
            logger.debug(f"Error in webhook task runner for meeting_id {meeting_id}: {e}", exc_info=True)
