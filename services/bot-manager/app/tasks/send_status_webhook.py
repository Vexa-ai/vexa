...
async def run(meeting: Meeting, db: AsyncSession, status_change_info: Optional[Dict[str, Any]] = None):
    ...
    """
    logger.debug(f"Executing send_status_webhook task for meeting {meeting.id} with status {meeting.status}")
    ...
    try:
        ...
        async with httpx.AsyncClient(follow_redirects=True) as client:
            logger.debug(f"Sending status webhook to {webhook_url} for meeting {meeting.id} (status: {meeting.status})")
            ...
            if response.status_code >= 200 and response.status_code < 300:
                logger.debug(f"Successfully sent status webhook for meeting {meeting.id} to {webhook_url}")
            else:
                logger.debug(f"Status webhook for meeting {meeting.id} returned status {response.status_code}: {response.text}")

    except httpx.RequestError as e:
        logger.debug(f"Failed to send status webhook for meeting {meeting.id}: {e}")
    except Exception as e:
        logger.debug(f"Unexpected error sending status webhook for meeting {meeting.id}: {e}", exc_info=True)
