...
try:
    from whisper_live.remote_transcriber import (
        RemoteTranscriber,
        RemoteTranscriberOverloaded,
    )
    from whisper_live.remote_transcriber_mistral import (
        RemoteTranscriber as RemoteTranscriberMistral,
    )
    ...
    RemoteTranscriberMistral = None

# Determine which transcriber to use based on TRANSCRIBER_PROVIDER
TRANSCRIBER_PROVIDER = os.getenv("TRANSCRIBER_PROVIDER", "Vexa").strip().upper()
if TRANSCRIBER_PROVIDER == "MISTRAL":
    RemoteTranscriber = RemoteTranscriberMistral
elif TRANSCRIBER_PROVIDER == "VEXA":
    pass  # Use default RemoteTranscriber
else:
    logging.warning(
        f"Unknown TRANSCRIBER_PROVIDER: {TRANSCRIBER_PROVIDER}. Defaulting to Vexa."
    )
...
class ServeClientFasterWhisper(ServeClientBase):
    ...
    def __init__(
        ...
        if self.model_size_or_path.endswith("en"):
            self.language = "en"
            self.language_provided = (
                True  # Model-based language is considered "provided"
            )
        else:
            self.language = language
        ...
        logging.info(
            f"FasterWhisper client {client_uid}: min_audio_s={self.min_audio_s} (server_options had: {server_options.get('min_audio_s', 'NOT SET')})"
        )
    ...
    def transcribe_audio(self, input_sample):
        ...
        language_detection_segments = (
            1
            if not self.language_provided
            else int(os.getenv("LANGUAGE_DETECTION_SEGMENTS", "10"))
        )
    ...
    def handle_transcription_output(self, result, duration):
        ...
        if len(result):
            ...
            if last_segment and not last_segment.get("completed", True):
                logging.info(
                    f"SEGMENT_UPDATE: client={self.client_uid}, partial_segment={last_segment.get('text', '')[:50]}, start={last_segment.get('start')}, end={last_segment.get('end')}"
                )
        else:
        ...
        while True:
            ...
            try:
                ...
                if result is None or (
                    not self.language_provided and self.language is None
                ):
                    self.timestamp_offset += duration
                    time.sleep(
                        0.25
...
class ServeClientRemote(ServeClientBase):
    def __init__(
        ...
        # Log the critical parameters
        logging.info(
            f"Initializing Remote client {client_uid} with platform={platform}, meeting_url={meeting_url}, token={token}"
        )
        ...
        if hasattr(settings, "MIN_TIME_BETWEEN_REQUESTS_S"):
            ...
            default_frequency_hz = getattr(
                settings, "MAX_TRANSCRIPTION_FREQUENCY_HZ", 2.0
            )
            default_min_time = (
                1.0 / default_frequency_hz if default_frequency_hz > 0 else 0.0
            )
        else:
            default_min_time = 1.0

        # Check server_options for either old or new parameter name
        min_time = server_options.get(
        ...
        self.last_transcription_time = (
            0.0  # Track when last transcription request completed
        )

        logging.info(
            f"Remote client {client_uid}: min_audio_s={self.min_audio_s} (server_options had: {server_options.get('min_audio_s', 'NOT SET')})"
        )
        logging.info(
            f"Remote client {client_uid}: same_output_threshold={self.same_output_threshold} (server_options had: {server_options.get('same_output_threshold', 'NOT SET')})"
        )
        logging.info(
            f"Remote client {client_uid}: min_time_between_requests={self.min_time_between_requests:.3f}s (max {1.0 / self.min_time_between_requests:.2f} requests/second)"
        )
        ...
        if not REMOTE_AVAILABLE:
            logging.error(
                "Remote transcriber is not available. Please install requests package and set REMOTE_TRANSCRIBER_* environment variables."
            )
            self.websocket.send(
                json.dumps(
                    {
    ...
    def create_model(self):
        ...
        logging.debug(
            f"Creating RemoteTranscriber with API key: {api_key_masked}, URL: {api_url}, Model: {model}"
        )

        # Use the selected transcriber provider
        if TRANSCRIBER_PROVIDER == "MISTRAL":
            try:
                from whisper_live.remote_transcriber_mistral import (
                    RemoteTranscriber as RemoteTranscriberMistral,
                ...
                self.transcriber = RemoteTranscriberMistral(
                    api_url=api_url,
                    api_key=api_key,
                    model=model,
                ...
                logging.error(f"Failed to create Mistral transcriber: {e}")
                raise
        else:
            # Default to Vexa
    ...
    def speech_to_text(self):
        ...
        while True:
            ...
            with self.transcription_lock:
                ...
                self.clip_audio_if_no_valid_segment()
                latest_input_bytes, latest_duration = (
                    self.get_audio_chunk_for_processing()
                )
            ...
            if self.min_time_between_requests > 0:
                ...
                if time_since_last < self.min_time_between_requests:
                    ...
                    logging.info(
                        f"RATE_LIMIT: Waiting {wait_time:.3f}s before next transcription request (last was {time_since_last:.3f}s ago, min interval is {self.min_time_between_requests:.3f}s)"
                    )
                    ...
                    with self.transcription_lock:
                        ...
                        if latest_duration >= self.min_audio_s:
                            ...
                            logging.info(
                                f"LIFO: After rate limit wait, processing latest audio chunk (duration={latest_duration:.2f}s)"
                            )
                        else:
                            # Not enough audio after wait, clear in-flight flag and continue
                            logging.debug(
                                f"LIFO: After rate limit wait, not enough audio (duration={latest_duration:.2f}s < min={self.min_audio_s:.2f}s)"
                            )
                    ...
                    with self.transcription_lock:
                        ...
                        if latest_duration >= self.min_audio_s:
                            ...
                            current_duration = latest_duration
                            logging.info(
                                f"LIFO: Processing latest audio chunk (duration={latest_duration:.2f}s)"
                            )
                        else:
                            # Not enough audio, clear in-flight flag and continue
                            logging.debug(
                                f"LIFO: Not enough audio (duration={latest_duration:.2f}s < min={self.min_audio_s:.2f}s)"
                            )
                            self.transcription_in_flight = False
                            continue
            ...
            try:
                ...
                if result is None or (
                    ...
                    logging.info(
                        f"ALGORITHM_A: VAD silence detected (result=None or language=None), cutting buffer by {current_duration:.3f}s"
                    )
                    time.sleep(
                        0.25
                    )  # wait for voice activity, result is None when no voice activity
                    ...
                    try:
                        if hasattr(result, "__len__") and len(result) == 0:
                            with self.lock:
                                self.timestamp_offset += current_duration
                            logging.info(
                                f"ALGORITHM_A: VAD silence detected (empty segments), cutting buffer by {current_duration:.3f}s"
                            )
                            time.sleep(0.25)
                        else:
                ...
                with self.transcription_lock:
                    if latest_duration >= self.min_audio_s:
                        ...
                        logging.info(
                            f"LIFO: Response received, latest audio available (duration={latest_duration:.2f}s), will process on next iteration"
                        )
                        ...
                        logging.debug(
                            f"LIFO: Response received, no new audio available (duration={latest_duration:.2f}s < min={self.min_audio_s:.2f}s)"
                        )
                        self.transcription_in_flight = False
                ...
                if RemoteTranscriberOverloaded is not None and isinstance(
                    e, RemoteTranscriberOverloaded
                ):
                    retry_after = getattr(e, "retry_after_s", 0.5) or 0.5
                    logging.info(
                        f"Remote transcriber overloaded; backing off {retry_after:.2f}s (HTTP {getattr(e, 'status_code', '??')})"
                    )
                    with self.transcription_lock:
                        self.transcription_in_flight = False
    ...
    def update_segments(self, segments, duration):
        ...
        if (
            len(segments_list) > 1
            and segments_list[-1].no_speech_prob <= self.no_speech_thresh
        ):
        ...
        if (
            ...
            if filtered_current_out is not None:
                ...
                try:
                    if WL_LOG_HALLUCINATIONS:
                        logger.info(
                            f'HALLUCINATION_FILTERED: "{segments_list[-1].text}"'
                        )
                except Exception:
                    pass
                last_segment = None
        ...
        if self.current_out.strip() == self.prev_out.strip() and self.current_out != "":
            ...
            # audio thats not yet transcribed so, capturing the time when it was repeated for the first time
            if self.end_time_for_same_output is None:
                self.end_time_for_same_output = (
                    segments_list[-1].end if segments_list else duration
                )
            time.sleep(
                0.1
            )  # wait for some voice activity just in case there is an unitended pause from the speaker for better punctuations.
                ...
                if filtered_current_out is not None:
                    ...
                    with self.lock:
                        start, end = (
                            self.timestamp_offset,
                            self.timestamp_offset
                            + min(duration, self.end_time_for_same_output or duration),
                        )
                    if start < end:
                        ...
                        logging.info(
                            f"SAME_OUTPUT_THRESHOLD: client={self.client_uid}, completed_segment={filtered_current_out[:50]}, start={start:.3f}, end={end:.3f}, count={self.same_output_count}"
                        )
        ...
        if offset is not None:
            with self.lock:
                ...
                logging.debug(
                    f"ALGORITHM_A: Advanced timestamp_offset by {offset:.3f}s (reason={'SAME_OUTPUT_THRESHOLD' if threshold_reached else 'completed_segments'})"
                )
        else:
            # No advancement - buffer will accumulate more audio and be re-transcribed
            logging.debug(
                f"ALGORITHM_A: No offset advancement (no completed segments, threshold not reached, buffer will accumulate)"
            )
    ...
    def set_language(self, info):
        ...
        if info and hasattr(info, "language"):
            ...
            logger.info(
                f"LANGUAGE_DETECTION: client={self.client_uid}, language={self.language}, confidence={lang_prob:.4f}"
            )
