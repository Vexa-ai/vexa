...
function runAws(
  ...
  execFileSync('aws', args, { env, stdio, timeout });
}
...
export function s3Sync(localDir: string, s3Path: string, config: S3Config, direction: 'up' | 'down', excludes: string[] = []): void {
  ...
  const excludeArgs = excludes.flatMap((exclude) => ['--exclude', exclude]);
  runAws(
    ['s3', 'sync', src, dst, '--endpoint-url', config.s3Endpoint, ...excludeArgs],
    getS3Env(config),
    'inherit',
    300000
  );
...
export function syncBrowserDataToS3(config: S3Config): void {
  ...
  for (const file of AUTH_ESSENTIAL_FILES) {
    ...
    try {
      runAws(['s3', 'cp', local, `${s3Base}/${file}`, '--endpoint-url', config.s3Endpoint], env, 'pipe', 10000);
      uploaded++;
    } catch (err: any) {
  ...
  for (const dir of AUTH_ESSENTIAL_DIRS) {
    ...
    if (!existsSync(local)) continue;
    try {
      runAws(['s3', 'sync', `${local}/`, `${s3Base}/${dir}/`, '--endpoint-url', config.s3Endpoint], env, 'pipe', 10000);
      uploaded++;
    } catch (err: any) {
