...
import type { InitialMeetingsPage } from "@/lib/meeting-list";
...
interface MeetingsClientProps {
  initialPage: InitialMeetingsPage;
}

export default function MeetingsClient({ initialPage }: MeetingsClientProps) {
  usePendingMeeting();
  ...
  const [isUsingInitialPage, setIsUsingInitialPage] = useState(
    initialPage.state !== "fallback",
  );
  ...
  // Render the server-loaded first page immediately. Only a failed preload
  // uses the existing client request path.
  useEffect(() => {
    if (initialPage.state === "fallback") {
      applyFilters("", "all", "all");
      return;
    }
    hydrateMeetings(initialPage);
    setIsUsingInitialPage(false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const initialFilterPass = useRef(true);
  useEffect(() => {
    if (initialFilterPass.current) {
      initialFilterPass.current = false;
      return;
    }
    setIsUsingInitialPage(false);
  ...
  const visibleMeetings = isUsingInitialPage ? initialPage.meetings : meetings;
  const visibleHasMore = isUsingInitialPage ? initialPage.hasMore : hasMore;
  const visibleSubscriptionRequired = isUsingInitialPage
    ? initialPage.state === "subscription_required"
    : subscriptionRequired;
  ...
  return (
    <div className="space-y-6">
      ...
      ) : visibleSubscriptionRequired && visibleMeetings.length === 0 ? (
        <ErrorState
          type="subscription"
                  ...
                  <tr>
                    <td colSpan={6} className="px-5 py-12 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        {!searchQuery.trim() && platformFilter === "all" && statusFilter === "all" && !visibleSubscriptionRequired && (
