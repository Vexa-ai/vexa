...
async def transcribe_meeting(
    ...
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            form_data = {"model": "whisper-large-v3-turbo", "response_format": "verbose_json"}
            if req.language:
    ...
    # Groq returns language as full name (e.g. 'English'); map to ISO 639-1
    _LANG_NAME_TO_ISO = {
        'english':'en','spanish':'es','french':'fr','german':'de','italian':'it',
        'portuguese':'pt','dutch':'nl','russian':'ru','chinese':'zh','japanese':'ja',
        'korean':'ko','arabic':'ar','hindi':'hi','turkish':'tr','polish':'pl',
        'catalan':'ca','swedish':'sv','danish':'da','norwegian':'no','finnish':'fi',
    }
    detected_language = _LANG_NAME_TO_ISO.get(detected_language.lower(), detected_language if len(detected_language)<=3 else 'unknown')
