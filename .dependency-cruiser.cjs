...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      severity: "error",
      from: { path: "^meetings/(services|modules)/" },
      to: { path: "^agent/(services|modules)/" },
    },
    {
      name: "agent-internals-not-meetings",
      comment: "Seam — agent internals must not import meetings internals (contracts/ is allowed).",
      severity: "error",
      from: { path: "^agent/(services|modules)/" },
      to: { path: "^meetings/(services|modules)/" },
    },
    {
      ...
      severity: "error",
      from: { path: "^runtime/(services|modules|src)/" },
      to: { path: "^(meetings|agent|identity|gateway|integrations|clients|sdks)/(services|modules)/" },
    },
  ],
  options: {
