...
- Post-code-review trust UX fix `898b261 fix(dashboard): neutralize "missing recording" red banner`: replaces the destructive red banner with neutral "Recording is finalizing..." during the normal post-meeting window. Same dashboard meeting page surface as pack 1.
...
- Post-stitch evidence-only regression fix `5acf36c fix(pack-1): use @example.com (RFC 2606) instead of @example.invalid (RFC 6761)`: pack 1's `ops/lite_playback_smoke.py` seeded a user with `@example.invalid`, which Pydantic v2 EmailStr rejects; the stitched 0.10.6.3 candidate's `GET /admin/users` then 500'd for all clients. No product code touched.
