#!/usr/bin/env node
/**
 * arch-report.mjs — generate docs/ARCH-COMPLIANCE.md: each MODULARITY principle → the CI gate(s) that
 ...
 *
 *   node scripts/arch-report.mjs           # (re)generate docs/ARCH-COMPLIANCE.md
...
if (!CHECK) {
  const md = [
    "# Architecture compliance — modularity (generated)",
    "",
    "> Generated by `scripts/arch-report.mjs`. The constitution promises every principle names the practice",
    "> it comes from **and the gate that enforces it** ([`ARCHITECTURE.md`](ARCHITECTURE.md) §2/§4, **P9**: an",
  ...
  writeFileSync(join(ROOT, "docs", "ARCH-COMPLIANCE.md"), md);
  console.log(`arch-report → docs/ARCH-COMPLIANCE.md (${allOk ? "all modularity gates green" : "SOME RED"})`);
} else if (allOk) {
