...
type Providers = { google: boolean; microsoft: boolean; keycloak: boolean };

export function AuthGate({ children }: { children: React.ReactNode }) {
  const [status, setStatus] = useState<Status>("checking");
  const [providers, setProviders] = useState<Providers>({ google: false, microsoft: false, keycloak: false });
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/providers", { cache: "no-store" })
      ...
      .then((p: Record<string, unknown>) =>
        active && setProviders({ google: !!p.google, microsoft: !!p.microsoft, keycloak: !!p.keycloak }))
      .catch(() => undefined);
