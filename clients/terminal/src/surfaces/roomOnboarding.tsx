...
export function RoomOnboarding({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const [msgs, setMsgs] = useState<Msg[]>([
    { who: "a", text: "Let’s set up a room. What should it be called?\n\nA room usually carries the name of the meeting it serves — “Architecture review”, “Acme renewal”, “Monday standup”." },
  ]);
  ...
  const submitText = () => {
    ...
    if (phase === "focus") {
      ...
      say("Good. Last thing — who belongs in this room?");
      setPhase("who");
      return;
    }
    if (phase === "emails") {
  ...
  return (
    <Modal title="New shared room" onClose={onClose} width={470}>
      <div style={{ display: "flex", flexDirection: "column", height: 430 }}>
        ...
        {phase === "focus" && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7, padding: "10px 0 2px" }}>
            {chip("Looks right", () => { setInput("looks right"); setTimeout(() => { const t = "looks right"; setMsgs((m) => [...m, { who: "u", text: t }]); say("Good. Last thing — who belongs in this room?"); setPhase("who"); setInput(""); }, 0); }, true)}
          </div>
        )}
        {phase === "done" ? (
