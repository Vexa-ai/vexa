...
describe("scaffoldToChat", () => {
  ...
  it("phase null keeps the meeting's own layout and never infers `post`", () => {
    ...
    expect(rec.artifacts[0]).toEqual({ path: "kg/entities/meeting/2026-03-02-show-b-lighting-dailies.md", label: "Brief" });
  });
...
describe("localScaffold — the hand link composes through the SAME path", () => {
  it("`?ask=&meeting=` produces a record scaffoldToChat renders identically", () => {
    ...
    expect(rec.artifacts).toEqual([{ path: "kg/entities/meeting/2026-03-02-show-b-lighting-dailies.md", label: "Brief" }]);
  });
  ...
  it("a hand link with no note path opens the transcript alone, never a dead Brief tab", () => {
    ...
    expect(rec.focus).toBeUndefined();   // focus drops with the token it named
  });
});
