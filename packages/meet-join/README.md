...
- `make debug-cloud URL=<meet-url> CLOUD_HOST=<host>` — same image from another network position (SSH tunnel). `bbb` = second household vantage; a throwaway Linode VM = the production position (prod bots speak from datacenter ranges) — necessary for #444 block-path reproduction
