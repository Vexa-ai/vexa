...
The DB URL is `postgresql+pg8000://user:password@postgres:5432/flows` in `VEXA_FLOWS_DB_URL`.
pg8000 is BSD-3-Clause (Category A), pure Python, and needs no libpq; `db.py` translates
the URL's optional `sslmode` for both the runtime and deployment bootstrap.
