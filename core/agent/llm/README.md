...
- **Completions**: `openai_compat.py` (DEFAULT — OpenRouter, Ollama, vLLM, LM Studio, OpenAI, any
  ...
  `orca.py` (a named [OrcaRouter](https://www.orcarouter.ai) gateway — OpenAI-compatible with a
  default host, opt-in via `VEXA_LLM_PROVIDER=orcarouter`; no endpoint to type) ·
...
|---|---|---|
| `VEXA_LLM_PROVIDER` | completion adapter: `openai-compat` \| `anthropic` \| `orcarouter` \| `claude-cli` | `openai-compat` |
| `VEXA_LLM_BASE_URL` | provider endpoint | anthropic: `https://api.anthropic.com`; orcarouter: `https://api.orcarouter.ai/v1`; openai-compat: **required** (falls back to `ANTHROPIC_BASE_URL`) |
...
- Vendor names appear only in adapter files (`claude_code.py`, `anthropic_api.py`, `orca.py`),
  never in `ports.py`/`registry.py` beyond registry keys.
