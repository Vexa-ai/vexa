...
class JobRunner:
    ...
    def __init__(self, *, emit: Callable[[dict], None], turn: Callable[[str], Iterator[dict]],
                 register_dir: Optional[Path] = None) -> None:
        self._emit = emit
        self._turn = turn
    ...
    def spawn(self, kind: str, target: str, brief: str, *, turn_id: str = "") -> dict:
        ...
        with self._lock:
            if key in self._running:
                ev = {"type": "job-refused", "kind": kind, "target": target,
        ...
        ev = {"type": "job-started", "job_id": job_id, "kind": kind, "target": target,
    ...
    def cancelled_at_boot(self) -> list[dict]:
        ...
        for path in sorted(self._dir.glob("*.json")):
            ...
            ev = {"type": "job-failed", "job_id": str(rec.get("job_id") or path.stem),
    ...
    def _run(self, job_id: str, key: str, kind: str, target: str, brief: str) -> None:
        ...
        try:
            ...
            self._forget(job_id)
            self._emit({"type": "job-done" if ok else "job-failed", "job_id": job_id,
