...
export interface OrchestratorDeps {
  ...
  /** Optional — the forensics a TERMINAL event carries and no other: what DEGRADED the meeting
   *  without ending it (STT refusing every chunk) and WHO was in the room (attendance). Consulted
   *  once, when a terminal event is built, and merged onto it. A meeting whose STT backend was
   *  dead used to reach `completed` indistinguishable from a silent room: the faults were typed
   *  and attributed all the way to the composition root and then died in a console.error, so the
   *  emptiest possible transcript arrived with no reason attached (the #807 shape). Rides
   *  lifecycle.v1 exactly as `infra_fault` does — the contract is additionalProperties:true, so
   *  this is additive.
   *  Returns `undefined` when there is nothing to add. MUST NOT throw. */
  terminalExtras?: () => Record<string, unknown> | undefined;
}
...
export function createOrchestrator(inv: Invocation, deps: OrchestratorDeps) {
  ...
  const emit = async (status: BotStatus, extra: Partial<LifecycleEvent> = {}): Promise<void> => {
    ...
    // or anyone was in the room, it says so here or the record dies with the container. A reporter
    // fault must never change the exit path (P18: report, but never at the cost of the report itself).
    let extras: Record<string, unknown> | undefined;
    if (isTerminal(status) && deps.terminalExtras) {
      try { extras = deps.terminalExtras(); } catch { extras = undefined; }
    }
    await deps.lifecycle.emit({ ...base, status, ...extra, ...(extras ?? {}) });
  };
