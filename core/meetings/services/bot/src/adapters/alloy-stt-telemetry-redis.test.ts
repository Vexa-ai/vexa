/**
 * ALLOY: Offline publisher lifecycle checks plus an explicit real-Redis lane.
 */
...
const realRedisRequested = process.argv.includes('--real-redis');
if (realRedisRequested) {
  const redisUrl = process.env.ALLOY_TEST_REDIS_URL?.trim();
  if (!redisUrl) {
    throw new Error(
      '[ALLOY] --real-redis requires a non-empty ALLOY_TEST_REDIS_URL for a disposable Redis database',
    );
  }
  ...
  console.log('SKIP ALLOY real-Redis integration (run the explicit test:redis lane)');
}
