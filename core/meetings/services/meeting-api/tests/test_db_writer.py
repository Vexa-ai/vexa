...
async def test_mixed_lane_confirmed_segment_flushes_without_waiting_out_the_hold(store, bus, redis_c):
    """The mixed lane (Teams/Zoom, source=merged) retracts drafts instead of refining rows in
    place, so a CONFIRMED segment of its is durable-safe the moment it exists. Measured cost of
    the old behaviour on prod meeting 26088: 36.2 s of a 39.4 s wait, against 2.0 s of model."""
    seg = {**_seg("turn:3:0", 1.0, "shipped now"), "source": "merged"}
...
async def test_mixed_lane_PENDING_still_waits_out_the_hold(store, bus, redis_c):
    ...
    seg = {**_seg("turn:3:p0", 1.0, "still forming", completed=False), "source": "merged"}
    await bus.xadd("transcription_segments", json.loads(_message(1, [seg])["payload"]))
    await consume_segments(store, bus)
