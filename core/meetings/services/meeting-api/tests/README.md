...
| `test_claims_audit.py` | O-MTG-1 | the lifecycle claims audit (`lifecycle/claims_audit.py`) flags exactly the inconsistent terminal-row shapes and exactly none of the consistent ones — driven by the synthetic corpus in `fixtures/claims_audit_rows.json`, asserting the exact ordered finding set per row in both directions. |
