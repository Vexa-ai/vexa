...
| `test_zaki_capture.py` | L2 unit | a short-lived authority is bound to tenant/user/exact meeting, URL-guarded, and intersects operator/tenant/user/quota before I/O; accepted capture forces `ZAKI Notetaker`, materializes three retention expiries, and runtime refusal becomes terminal. Withdrawal is scoped, idempotent, persisted before stop, directly tears down a booting workload, and cannot be resurrected by a late callback. |
| `test_zaki_capture_adapters.py` | L2 adapter | PostgreSQL withdrawal takes the exclusive meeting-write barrier before mutation, while a later transcript writer takes the shared side, observes `withdrawn`, and refuses before Redis. |
