...
| `test_zaki_capture.py` | L2 unit | a short-lived authority is bound to tenant/user/exact meeting, URL-guarded, and intersects operator/tenant/user/quota before I/O; accepted capture forces `ZAKI Notetaker`, materializes three retention expiries, and a runtime quota backstop becomes a named terminal non-capture. |
