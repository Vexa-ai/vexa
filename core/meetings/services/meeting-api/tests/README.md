...
| `test_zaki_capture.py` | L2 unit | a short-lived, single-use authority is bound to tenant/user/exact meeting, platform-approved URL hosts, and intersects operator/tenant/user/quota before I/O; accepted capture forces `ZAKI Notetaker`, materializes three retention expiries, rejects exact or differently identified stale grants, and keeps withdrawal monotonic through runtime failure races. Withdrawal is scoped, idempotent, persisted before stop, directly tears down a booting workload, and cannot be resurrected or inserted into the audit trail by a late callback. |
| `test_zaki_capture_adapters.py` | L2 adapter | PostgreSQL spawn and withdrawal share the per-user transaction lock, stale authority is rejected against the latest scope tombstone, and withdrawal takes the exclusive meeting-write barrier before mutation; Redis and durable PostgreSQL transcript writers take the shared side, observe `withdrawn`, and refuse before PII mutation. |
