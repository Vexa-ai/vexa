...
#
# WHY THE MIXED LANE DOES NOT NEED IT. The mixed lane (Teams/Zoom, ``source: 'merged'``) never
# refines in place: a draft carries a DISJOINT id (``turn:N:pI`` vs the confirmed ``turn:N:seq``)
# and is actively RETRACTED — ``transcript_retract`` → ``delete_segments`` DELETES the durable row
# as well as the hash field. A later rewrite of a CONFIRMED segment (publish-stitch merges into the
# previous turn's row, same id) re-enters the hash and lands as an UPSERT-UPDATE. So for a mixed
# CONFIRMED segment the hold buys nothing and costs the customer the whole 30 s: measured on prod
...
#
# The split is therefore on the two facts already in the hash entry — ``source`` (which lane wrote
# it, ``ingest.py``) and ``completed`` — and pendings keep the full hold in every lane, so a draft
# can never become a durable row that reads back as final.
MIXED_LANE_SOURCES = {"merged"}
IMMUTABILITY_THRESHOLD_MIXED_CONFIRMED = float(
    os.environ.get("IMMUTABILITY_THRESHOLD_MIXED_CONFIRMED", "0")
)
...
def threshold_for(seg: dict, default_threshold: float) -> float:
    ...
    returned unchanged for every lane that refines rows in place. Only a CONFIRMED segment written
    by the mixed lane — the lane whose drafts are retracted rather than refined — is allowed the
    shorter hold, and never a longer one than the caller asked for.
    """
    try:
        if seg.get("completed") is True and seg.get("source") in MIXED_LANE_SOURCES:
            return min(default_threshold, IMMUTABILITY_THRESHOLD_MIXED_CONFIRMED)
