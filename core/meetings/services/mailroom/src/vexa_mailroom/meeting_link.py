"""Meeting-link → ``(platform, native_meeting_id)`` parsing — the mailroom's VENDORED copy of
``meeting_api.collector.meeting_link`` (the same idiom ``vexa_mcp.link_parser`` follows: a
service never imports a sibling package's internals, P2/gate:isolation-py — it carries its own
copy of the pure logic).

**Keep the two in step.** The mailroom extracts the joinable link out of an ICS invite's
X-GOOGLE-CONFERENCE / LOCATION / DESCRIPTION and then hands ``meeting_url`` to ``POST /meetings``,
which parses it AGAIN with the meeting-api copy — a divergence would mean the mailroom accepted a
link the control plane then rejects with 422. ``tests/test_meeting_link.py`` pins the shapes both
copies must agree on.

Pure string logic — no I/O, no framework imports; the one config read is ``VEXA_JITSI_HOSTS``
(P14), consulted per call so tests and reloads see the live env.
