# Release validation report — `0.10.6-260512-1745`

_Generated 2026-05-12T15:07:26.126246Z from `tests3/.state/reports/`._
...
|-------|----------------|-------------------|---------|
| `finalizer-master-path-race` | compose | compose `SINGLE_WRITER_FOR_RECORDING_MASTER_PATH`: ⬜ missing<br>helm `SINGLE_WRITER_FOR_RECORDING_MASTER_PATH`: ⬜ missing | **⬜ missing** |
| `pr-319-swagger-header-fix` | compose | compose `SWAGGER_CURL_EXAMPLE_SHOWS_CORRECT_HEADER`: ⬜ missing | **⬜ missing** |
| `pr-239-camera-enabled-when-voice-agent` | compose | compose `BOT_CONFIG_CAMERA_ENABLED_WHEN_VOICE_AGENT`: ⬜ missing<br>helm `BOT_CONFIG_CAMERA_ENABLED_WHEN_VOICE_AGENT`: ⬜ missing | **⬜ missing** |
| `pr-283-teams-continue-no-av-modal` | compose | compose `TEAMS_CONTINUE_NO_AV_MODAL_DISMISSED`: ⬜ missing | **⬜ missing** |
| `vexa-lite-docs-env-hygiene` | lite | lite `VEXA_LITE_APPLE_SILICON_CAVEAT_DOCUMENTED`: ✅ pass | **✅ pass** |
| `gmeet-rejection-fast-fail-and-rejoin` | compose | compose `GMEET_REJECTION_PAGE_FAST_FAIL_UNDER_30S`: ⬜ missing<br>compose `GMEET_WAITING_ROOM_EVICTION_RETRY_OR_FAIL_CLEAN`: ⬜ missing | **⬜ missing** |
| `callbacks-broad-except-narrow` | compose | compose `CALLBACKS_FINALIZE_NARROW_EXCEPT`: ⬜ missing | **⬜ missing** |
| `chunk-write-prior-count-cosmetic-log` | compose | compose `CHUNK_WRITE_PRIOR_COUNT_LOG_ACCURATE`: ⬜ missing | **⬜ missing** |
| `stale-issue-audit-sweep` | any | — | **✅ pass** |
| `byo-tts-file-playback-validation` | compose | compose `TTS_PLAYBACK_HANDLES_WAV_AND_MP3_RESPONSES`: ⬜ missing | **⬜ missing** |
| `tts-auto-language-detection` | compose | compose `TTS_AUTO_LANG_PICKS_RIGHT_VOICE`: ⬜ missing<br>compose `TTS_NEW_LANG_VOICE_AUTO_DOWNLOAD_CACHED`: ⬜ missing | **⬜ missing** |
...
| `drop-relational-recordings-tables` | (any) | compose `RECORDINGS_TABLE_NOT_REFERENCED`: ✅ pass<br>compose `MEDIA_FILES_TABLE_NOT_REFERENCED`: ✅ pass<br>compose `RECORDINGS_TABLE_DROPPED_IN_PROD`: ⬜ missing<br>helm `RECORDINGS_TABLE_DROPPED_IN_PROD`: ⬜ missing | **⬜ missing** |
...
|------|-----------|-----------|--------|--------|
| `compose` | `0.10.6-260512-1745` | 5 | 5 | 0 |
| `helm` | `—` | 0 | 0 | 0 |
| `lite` | `0.10.6-260512-1745` | 9 | 9 | 0 |
...
| `security-hygiene` | **33%** | 95% | ❌ below gate |
...
| graceful-leave | Bot leaves the meeting gracefully on stop (no force-kill by default) | 5 | ⬜ missing | lite: smoke-static/GRACEFUL_LEAVE: self_initiated_leave during stopping treated as completed, not failed; compose: check GRACEFUL_LEAVE not found in any report; helm: check GRACEFUL_LEAVE not found… |
| route-collision | No Starlette route collisions — /bots/{id} and /bots/{platform}/{native_id} do not clash | 5 | ⬜ missing | lite: smoke-static/ROUTE_COLLISION: bot detail route is /bots/id/{id}, not /bots/{id} which collides with /bots/status; compose: check ROUTE_COLLISION not found in any report; helm: check ROUTE_COL… |
...
| cookie-security | HttpOnly + SameSite cookies on magic-link send/verify + admin-verify + nextauth | 10 | ⬜ missing | lite: smoke-static/SECURE_COOKIE_SEND_MAGIC_LINK: cookie Secure flag based on actual protocol, not NODE_ENV (send-magic-link); compose: check SECURE_COOKIE_SEND_MAGIC_LINK not found in any report; … |
| login-redirect | Magic-link click redirects to /meetings (not disabled /agent) | 5 | ⬜ missing | lite: smoke-static/LOGIN_REDIRECT: login redirects to / (then /meetings), not to disabled /agent page; compose: check LOGIN_REDIRECT not found in any report; helm: check LOGIN_REDIRECT not found in… |
| identity-no-fallback | /api/auth/me uses only the cookie for identity, never env fallback | 5 | ⬜ missing | lite: smoke-static/IDENTITY_NO_FALLBACK: /api/auth/me uses only cookie for identity, never falls back to env var; compose: check IDENTITY_NO_FALLBACK not found in any report; helm: check IDENTITY_N… |
...
| dashboard-audio-streams-from-bucket | dashboard reads /recordings/.../download (NOT /raw); <audio src> binds to the presigned URL; native HTTP Range fires on user seek | 10 | ⬜ missing | lite: v0.10.6-static-greps/DASHBOARD_AUDIO_STREAMS_FROM_BUCKET: dashboard reads /download → presigned URL; compose: check DASHBOARD_AUDIO_STREAMS_FROM_BUCKET not found in any report; helm: check DA… |
...
| gateway-timeout | Gateway proxy timeout is ≥30s (prevents premature 504s under load) | 10 | ⬜ missing | lite: smoke-static/GATEWAY_TIMEOUT_ADEQUATE: API gateway HTTP client timeout >= 15s — browser session creation needs time; compose: check GATEWAY_TIMEOUT_ADEQUATE not found in any report; helm: che… |
...
| # | Label | Weight | Status | Evidence |
|---|-------|-------:|:------:|----------|
| url-parser-exists | meeting-api has a URL parser module (url_parser.py) that handles platform detection | 10 | ⬜ missing | lite: smoke-static/URL_PARSER_EXISTS: MeetingCreate schema has parse_meeting_url — accepts meeting_url field directly; compose: check URL_PARSER_EXISTS not found in any report; helm: check URL_PARS… |
...
| # | Label | Weight | Status | Evidence |
|---|-------|-------:|:------:|----------|
| cdp-ws-scheme-preserved | CDP proxy webSocketDebuggerUrl rewrite preserves the inbound scheme (wss:// on HTTPS gateways) | 10 | ⬜ missing | lite: smoke-static/CDP_WS_SCHEME_PRESERVED: CDP proxy webSocketDebuggerUrl rewrite preserves inbound scheme (wss:// on HTTPS gateways) — Playwright connectOverCDP works through TLS; compose: check … |
| cdp-no-slash-redirect | Bare /b/{token}/cdp (no trailing slash) is a first-class route — no 307 scheme downgrade | 10 | ⬜ missing | lite: smoke-static/CDP_NO_SLASH_REDIRECT: Bare /b/{token}/cdp (no trailing slash) is a first-class route — no 307 scheme downgrade; compose: check CDP_NO_SLASH_REDIRECT not found in any report |

### `security-hygiene` (33% / gate 95%)

| # | Label | Weight | Status | Evidence |
|---|-------|-------:|:------:|----------|
| h11-pinned-safe | Every service requirements*.txt with httpx/uvicorn pins h11>=0.16.0 (CVE-2025-43859) | 10 | ⬜ missing | lite: smoke-static/H11_PINNED_SAFE_EVERYWHERE: every httpx/uvicorn requirements*.txt pins h11>=0.16.0 — CVE-2025-43859 closed transitively; compose: check H11_PINNED_SAFE_EVERYWHERE not found in an… |
| docs-env-gated-everywhere | Every FastAPI app sets docs_url/redoc_url/openapi_url from VEXA_ENV — /docs default-deny on VEXA_ENV=production | 10 | ⬜ missing | lite: smoke-static/DOCS_ENV_GATED_EVERYWHERE: every FastAPI app reads VEXA_ENV and passes docs_url/redoc_url/openapi_url derived from it — /docs default-deny in production; compose: check DOCS_ENV_… |
| vexa-bot-no-high-npm-vulns | services/vexa-bot + services/vexa-bot/core npm audit reports 0 HIGH + 0 CRITICAL | 5 | ⬜ missing | lite: smoke-static/VEXA_BOT_NO_HIGH_NPM_VULNS: services/vexa-bot[basic-ftp=5.3.0] services/vexa-bot/core[no-lockfile]; compose: check VEXA_BOT_NO_HIGH_NPM_VULNS not found in any report |
...
| engine-pool-reset-on-return-rollback-explicit | meeting-api database.py engine sets pool_reset_on_return='rollback' — regression guard for idle-in-transaction leak defense | 10 | ⬜ missing | lite: smoke-static/ENGINE_POOL_RESET_ON_RETURN_ROLLBACK: meeting-api SQLAlchemy engine keeps pool_reset_on_return="rollback" — guards idle-in-transaction leak defense; compose: check ENGINE_POOL_RE… |
...
| compose-ports-loopback-only | deploy/compose/docker-compose.yml dev-mode ports publish to 127.0.0.1 only (postgres, minio, admin-api, runtime-api, mcp) | 10 | ⬜ missing | compose: check COMPOSE_PORTS_LOOPBACK_ONLY not found in any report |
...
|------|:------:|---------:|---------------------:|
| `dashboard-playback-canonical` | ✅ pass | 144 ms | 1 / 1 |
| `no-media-files-table-references` | ✅ pass | 1429 ms | 1 / 1 |
| `no-recordings-table-references` | ✅ pass | 1292 ms | 1 / 1 |
| `recording-finalizing-state` | ✅ pass | 96 ms | 1 / 1 |
| `recording-playback-url-canonical` | ✅ pass | 200 ms | 1 / 1 |
...
| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `dashboard-playback-canonical` | ✅ pass | 121 ms | 1 / 1 |
| `no-media-files-table-references` | ✅ pass | 1430 ms | 1 / 1 |
| `no-recordings-table-references` | ✅ pass | 1277 ms | 1 / 1 |
| `smoke-env` | ✅ pass | 332 ms | 7 / 7 |
| `smoke-static` | ✅ pass | 2493 ms | 91 / 92 |
| `v0.10.5.3-hallucination-corpus` | ✅ pass | 337 ms | 3 / 3 |
| `v0.10.6-static-greps` | ✅ pass | 1279 ms | 15 / 15 |
| `v0.10.6.1-dispatch-check-noop-noop_when_unset` | ✅ pass | 264 ms | 1 / 1 |
| `v0.10.6.1-static-greps` | ✅ pass | 724 ms | 9 / 9 |
