# Release validation report — `0.10.5.3-260501-1430`

_Generated 2026-05-01T11:48:57.912934Z from `tests3/.state/reports/`._
...
|------|-----------|-----------|--------|--------|
| `compose` | `0.10.0-260421-2337` | 19 | 18 | 1 |
| `helm` | `0.10.5.3-260501-1430` | 12 | 8 | 4 |
| `lite` | `0.10.0-260421-2337` | 10 | 8 | 2 |
...
| status-completed | Meeting.status=completed after stop (not failed/stuck) | 10 | ❌ fail | helm: containers/status_completed: status=failed reason= (expected completed/gone, OR failed+clean-stop) after ~24x5s poll |
...
| dashboard-ws-url | NEXT_PUBLIC_WS_URL is set — live updates can connect | 5 | ✅ pass | lite: smoke-health/DASHBOARD_WS_URL: ws://localhost:3000/ws; compose: smoke-health/DASHBOARD_WS_URL: ws://localhost:3001/ws; helm: smoke-health/DASHBOARD_WS_URL: ws://172.236.127.147:30001/ws |
...
| chart-rolling-update-zero-surge | every app-facing Deployment in rendered chart has strategy.rollingUpdate.maxSurge: 0 — rolling updates never double DB-pool footprint | 10 | ❌ fail | helm: chart-rolling-update-zero-surge/HELM_ROLLING_UPDATE_ZERO_SURGE: admin-api:missing maxSurge 0 api-gateway:missing maxSurge 0 mcp:missing maxSurge 0 meeting-api:missing maxSurge 0 runtime-api:m… |
...
|------|:------:|---------:|---------------------:|
| `bot-records-incrementally` | ✅ pass | 102 ms | 1 / 1 |
| `containers` | ✅ pass | 109129 ms | 6 / 7 |
| `dashboard-auth` | ✅ pass | 543 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 1080 ms | 5 / 6 |
| `recording-survives-sigkill` | ✅ pass | 83 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 299 ms | 4 / 4 |
| `smoke-contract` | ✅ pass | 91593 ms | 27 / 27 |
| `smoke-env` | ❌ fail | 582 ms | 6 / 7 |
| `smoke-health` | ✅ pass | 5281 ms | 13 / 17 |
| `smoke-static` | ✅ pass | 48 ms | 45 / 46 |
...
| `webhooks` | ✅ pass | 96917 ms | 10 / 10 |

### `helm`

| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `chart-all-services-db-pool-tuned` | ✅ pass | 313 ms | 1 / 1 |
| `chart-pgbouncer-optional` | ✅ pass | 585 ms | 4 / 4 |
| `chart-prod-secrets-secretref` | ✅ pass | 394 ms | 2 / 2 |
| `chart-rolling-update-zero-surge` | ❌ fail | 253 ms | 0 / 1 |
| `containers` | ❌ fail | 235426 ms | 5 / 7 |
| `dashboard-auth` | ✅ pass | 1309 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 3281 ms | 4 / 6 |
| `smoke-contract` | ❌ fail | 28887 ms | 25 / 27 |
| `smoke-env` | ✅ pass | 9394 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 32929 ms | 17 / 17 |
| `smoke-static` | ❌ fail | 2916 ms | 89 / 92 |
| `webhooks` | ✅ pass | 103190 ms | 10 / 10 |

### `lite`

| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `bot-records-incrementally` | ✅ pass | 106 ms | 1 / 1 |
| `containers` | ❌ fail | 25416 ms | 2 / 2 |
| `dashboard-auth` | ✅ pass | 429 ms | 4 / 4 |
| `package-tests` | ✅ pass | 78 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 255 ms | 4 / 4 |
| `smoke-contract` | ✅ pass | 39516 ms | 22 / 27 |
| `smoke-env` | ✅ pass | 868 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 5859 ms | 12 / 17 |
| `smoke-static` | ✅ pass | 34 ms | 45 / 46 |
| `webhooks` | ❌ fail | 96433 ms | 9 / 10 |
