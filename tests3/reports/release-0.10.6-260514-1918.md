# Release validation report — `0.10.6-260514-1918`

_Generated 2026-05-14T16:48:15.286075Z from `tests3/.state/reports/`._
...
| `local-stack-walkability-smoke-gate` | compose, lite | compose `WALKABILITY_AUTH_ROUND_TRIP_WORKS`: ✅ pass<br>lite `WALKABILITY_AUTH_ROUND_TRIP_WORKS`: ✅ pass<br>compose `WALKABILITY_MEETINGS_PAGE_HAS_DATA`: ✅ pass<br>lite `WALKABILITY_MEETINGS_PAGE_HAS_DATA`: ✅ pass<br>compose `WALKABILITY_MEETING_DETAIL_API_LOADS`: ✅ pass<br>lite `WALKABILITY_MEETING_DETAIL_API_LOADS`: ✅ pass<br>compose `WALKABILITY_TRANSCRIPTION_URL_CONFIGURED`: ✅ pass<br>lite `WALKABILITY_TRANSCRIPTION_URL_CONFIGURED`: ✅ pass<br>compose `TRANSCRIPTION_TOKEN_NO_PLACEHOLDER_FALLBACK`: ✅ pass<br>lite `TRANSCRIPTION_TOKEN_NO_PLACEHOLDER_FALLBACK`: ✅ pass<br>compose `DASHBOARD_CONFIG_NO_STALE_LOCALHOST_DEFAULTS`: ⬜ missing<br>lite `DASHBOARD_CONFIG_NO_STALE_LOCALHOST_DEFAULTS`: ⬜ missing<br>compose `DASHBOARD_REWRITES_REQUIRE_BUILD_SSOT`: ⬜ missing<br>lite `DASHBOARD_REWRITES_REQUIRE_BUILD_SSOT`: ⬜ missing<br>compose `DASHBOARD_ADMIN_URL_EXPLICIT_SSOT`: ⬜ missing<br>lite `DASHBOARD_ADMIN_URL_EXPLICIT_SSOT`: ⬜ missing<br>compose `DASHBOARD_CLIENT_URLS_FROM_RUNTIME_CONFIG`: ⬜ missing<br>lite `DASHBOARD_CLIENT_URLS_FROM_RUNTIME_CONFIG`: ⬜ missing<br>compose `TEST_ACCOUNT_BOT_LIMIT_IS_CANONICAL`: ✅ pass<br>lite `TEST_ACCOUNT_BOT_LIMIT_IS_CANONICAL`: ✅ pass<br>compose `TEST_ACCOUNT_NO_STALE_SLOT_CONSUMERS`: ✅ pass<br>lite `TEST_ACCOUNT_NO_STALE_SLOT_CONSUMERS`: ✅ pass<br>compose `WALKABILITY_TTS_SPEAK_ROUND_TRIP`: ✅ pass<br>lite `WALKABILITY_TTS_SPEAK_ROUND_TRIP`: ✅ pass<br>compose `WALKABILITY_DASHBOARD_LOGIN_RENDERS`: ✅ pass<br>lite `WALKABILITY_DASHBOARD_LOGIN_RENDERS`: ✅ pass | **⬜ missing** |
...
|------|-----------|-----------|--------|--------|
| `any` | `—` | 0 | 0 | 0 |
| `compose` | `0.10.6-260514-0008` | 50 | 41 | 5 |
| `helm` | `0.10.6-260514-1918` | 29 | 22 | 7 |
| `lite` | `0.10.6-260514-0027` | 27 | 23 | 3 |
...
| `bot-lifecycle` | **89%** | 90% | ❌ below gate |
| `browser-session` | **0%** | 0% | ✅ pass |
| `container-lifecycle` | **0%** | 0% | ✅ pass |
| `dashboard` | **49%** | 90% | ❌ below gate |
...
| status-completed | Meeting.status=completed after stop (not failed/stuck) | 10 | ❌ fail | helm: containers/status_completed: status=stopping reason= stop_requested=1 (expected completed/gone, OR failed+clean-stop, OR failed+stop_requested=1) after ~24x5s poll |
...
### `dashboard` (49% / gate 90%)

| # | Label | Weight | Status | Evidence |
|---|-------|-------:|:------:|----------|
| login-flow | POST /api/auth/send-magic-link → 200 + success=true + sets vexa-token cookie | 10 | ❌ fail | lite: dashboard-auth/login: 200 + success=true; compose: dashboard-auth/login: 200 + success=true; helm: dashboard-auth/login: POST /api/auth/send-magic-link failed: {"error":"Email authentication … |
| cookie-flags | vexa-token cookie Secure flag matches deployment (Secure iff https) | 10 | ⬜ missing | lite: dashboard-auth/cookie_flags: flags correct for http; compose: dashboard-auth/cookie_flags: flags correct for http; helm: report has no step=cookie_flags |
| identity-me | GET /api/auth/me returns logged-in user's email (never falls back to env) | 10 | ⬜ missing | lite: dashboard-auth/identity: /me returns test@vexa.ai; compose: dashboard-auth/identity: /me returns test@vexa.ai; helm: report has no step=identity |
...
| proxy-reachable | GET /api/vexa/meetings via cookie returns 200 | 10 | ⬜ missing | lite: dashboard-auth/proxy_reachable: /api/vexa/meetings → 200; compose: dashboard-auth/proxy_reachable: /api/vexa/meetings → 200; helm: report has no step=proxy_reachable |
| meetings-list | /api/vexa/meetings returns a meeting list through the dashboard proxy | 5 | ⬜ missing | compose: dashboard-proxy/meetings_list: 1 meetings; helm: no report for test=dashboard-proxy |
| pagination | limit/offset pagination works (no overlap between pages) | 5 | ⬜ missing | compose: dashboard-proxy/pagination: need >=4 meetings in DB, have 1; helm: no report for test=dashboard-proxy |
...
| # | Label | Weight | Status | Evidence |
|---|-------|-------:|:------:|----------|
| events-meeting-completed | meeting.completed fires on every bot exit (default-enabled) | 10 | ⬜ missing | compose: report has no step=e2e_completion |
...
| envelope-shape | Every webhook carries envelope: event_id, event_type, api_version, created_at, data | 10 | ⬜ missing | compose: report has no step=envelope |
| headers-hmac | X-Webhook-Signature = HMAC-SHA256(timestamp + '.' + payload) when secret is set | 10 | ⬜ missing | compose: report has no step=hmac |
| security-spoof-protection | Client-supplied X-User-Webhook-* headers cannot override stored config | 10 | ⬜ missing | compose: report has no step=spoof |
| security-secret-not-exposed | webhook_secret never appears in any API response (POST /bots, GET /bots/status) | 10 | ⬜ missing | compose: report has no step=no_leak_response |
| security-payload-hygiene | Internal fields (secret, url, container ids, delivery state) stripped from webhook payloads | 5 | ⬜ missing | compose: report has no step=no_leak_payload |
...
| flow-gateway-inject | Gateway injects validated webhook config into meeting.data on POST /bots | 15 | ❌ fail | compose: webhooks/inject: POST /bots returned HTTP 403 |
...
| `containers` | ❌ fail | 10591 ms | 1 / 2 |
| `dashboard-auth` | ✅ pass | 560 ms | 4 / 4 |
...
| `dashboard-proxy` | ✅ pass | 867 ms | 3 / 6 |
...
| `recording-survives-sigkill` | ✅ pass | 82 ms | 1 / 1 |
| `recordings-tables-dropped` | ✅ pass | 311 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 276 ms | 4 / 4 |
| `smoke-bot-transcription-roundtrip-roundtrip` | ✅ pass | 683 ms | 1 / 1 |
| `smoke-contract` | ❌ fail | 116277 ms | 24 / 27 |
| `smoke-env` | ❌ fail | 634 ms | 6 / 7 |
| `smoke-health` | ✅ pass | 5315 ms | 13 / 17 |
| `smoke-static` | ❌ fail | 766 ms | 90 / 92 |
| `test-account-bot-limit-compose` | ✅ pass | 406 ms | 2 / 2 |
| `v0.10.5.3-hallucination-corpus` | ✅ pass | 250 ms | 3 / 3 |
| `v0.10.6-static-greps` | ✅ pass | 1110 ms | 15 / 15 |
...
| `webhooks` | ❌ fail | 13470 ms | 1 / 2 |

### `helm`

| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `advisory-dependency-floors` | ✅ pass | 161 ms | 1 / 1 |
| `chart-all-services-db-pool-tuned` | ✅ pass | 322 ms | 1 / 1 |
| `chart-pgbouncer-optional` | ✅ pass | 568 ms | 4 / 4 |
| `chart-prod-secrets-secretref` | ✅ pass | 394 ms | 2 / 2 |
| `chart-rolling-update-zero-downtime` | ✅ pass | 250 ms | 1 / 1 |
| `containers` | ❌ fail | 240361 ms | 5 / 7 |
| `dashboard-auth` | ❌ fail | 469 ms | 0 / 1 |
| `dashboard-playback-canonical` | ✅ pass | 1389 ms | 1 / 1 |
| `deferred-transcribe-master-gate` | ✅ pass | 449 ms | 5 / 5 |
| `no-media-files-table-references` | ✅ pass | 1579 ms | 1 / 1 |
| `no-recordings-table-references` | ✅ pass | 1451 ms | 1 / 1 |
| `smoke-bot-transcription-roundtrip-roundtrip` | ❌ fail | 112 ms | 0 / 1 |
| `smoke-contract` | ❌ fail | 92333 ms | 21 / 27 |
| `smoke-env` | ✅ pass | 9061 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 34248 ms | 17 / 17 |
| `smoke-static` | ✅ pass | 2973 ms | 91 / 92 |
| `v0.10.5.3-hallucination-corpus` | ✅ pass | 335 ms | 3 / 3 |
| `v0.10.6-static-greps` | ❌ fail | 1292 ms | 14 / 15 |
| `v0.10.6.1-dispatch-check-admin-deny-admin_bots_endpoint_honors_deny` | ✅ pass | 143 ms | 1 / 1 |
| `v0.10.6.1-dispatch-check-deny-bots_endpoint_honors_deny` | ✅ pass | 176 ms | 1 / 1 |
| `v0.10.6.1-dispatch-check-noop-noop_when_unset` | ✅ pass | 168 ms | 1 / 1 |
| `v0.10.6.1-post-meeting-idempotency-fires_once_per_session` | ✅ pass | 167 ms | 1 / 1 |
| `v0.10.6.1-speak-e2e-speak_delivers_audio` | ✅ pass | 134 ms | 1 / 1 |
| `v0.10.6.1-static-greps` | ✅ pass | 770 ms | 9 / 9 |
| `v0.10.6.1-tts-auto-lang-detects_and_picks_voice` | ✅ pass | 2060 ms | 1 / 1 |
| `v0.10.6.1-tts-auto-lang-voice_download_caches` | ✅ pass | 619 ms | 1 / 1 |
| `v0.10.6.1-tts-pod-pod_ready_after_first_boot` | ❌ fail | 183 ms | 0 / 0 |
| `v0.10.6.1-webm-duration-ebml_duration_present` | ✅ pass | 152 ms | 1 / 1 |
| `webhooks` | ❌ fail | 103959 ms | 9 / 10 |
...
| `bot-records-incrementally` | ✅ pass | 119 ms | 1 / 1 |
| `containers` | ❌ fail | 25432 ms | 2 / 2 |
| `dashboard-auth` | ✅ pass | 476 ms | 4 / 4 |
...
| `package-tests` | ✅ pass | 90 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 321 ms | 4 / 4 |
| `smoke-bot-transcription-roundtrip-roundtrip` | ✅ pass | 944 ms | 1 / 1 |
| `smoke-contract` | ❌ fail | 64267 ms | 20 / 27 |
| `smoke-env` | ✅ pass | 879 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 5608 ms | 12 / 17 |
| `smoke-static` | ❌ fail | 716 ms | 90 / 92 |
| `test-account-bot-limit-lite` | ✅ pass | 414 ms | 2 / 2 |
| `v0.10.5.3-hallucination-corpus` | ✅ pass | 257 ms | 3 / 3 |
| `v0.10.6-static-greps` | ✅ pass | 998 ms | 15 / 15 |
