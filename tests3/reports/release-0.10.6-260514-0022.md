# Release validation report — `0.10.6-260514-0022`

_Generated 2026-05-13T21:24:16.861499Z from `tests3/.state/reports/`._
...
|------|-----------|-----------|--------|--------|
| `any` | `—` | 0 | 0 | 0 |
| `compose` | `0.10.6-260514-0008` | 32 | 31 | 1 |
| `helm` | `—` | 0 | 0 | 0 |
| `lite` | `0.10.6-260514-0022` | 14 | 13 | 1 |
| `none` | `0.10.6-260512-1745` | 10 | 10 | 0 |
...
| `security-hygiene` | **67%** | 95% | ❌ below gate |
| `speaking-bot` | **0%** | 0% | ✅ pass |
| `webhooks` | **18%** | 95% | ❌ below gate |
...
| bots-status-not-422 | GET /bots/status never returns 422 (schema stable under concurrent writes) | 5 | ⬜ missing | lite: smoke-contract/BOTS_STATUS_NOT_422: GET /bots/status returns 200 — no route collision with /bots/{meeting_id}; compose: smoke-contract/BOTS_STATUS_NOT_422: GET /bots/status returns 200 — no r… |
...
| dashboard-ws-url | NEXT_PUBLIC_WS_URL is set — live updates can connect | 5 | ⬜ missing | lite: smoke-health/DASHBOARD_WS_URL: ws://localhost:3100/ws; compose: smoke-health/DASHBOARD_WS_URL: ws://localhost:3001/ws; helm: check DASHBOARD_WS_URL not found in any report |
...
| db-schema | Database schema is aligned with the current model | 10 | ⬜ missing | lite: smoke-health/DB_SCHEMA_ALIGNED: all required columns present; compose: smoke-health/DB_SCHEMA_ALIGNED: all required columns present; helm: check DB_SCHEMA_ALIGNED not found in any report |
...
| gmeet-parsed | Google Meet URL (meet.google.com/xxx-xxxx-xxx) parses correctly | 15 | ⬜ missing | lite: smoke-contract/GMEET_URL_PARSED: Google Meet URL accepted by POST /bots — parser handles GMeet format; compose: smoke-contract/GMEET_URL_PARSED: Google Meet URL accepted by POST /bots — parse… |
| invalid-rejected | Invalid meeting URL returns 400 (not 500) | 10 | ⬜ missing | lite: smoke-contract/INVALID_URL_REJECTED: garbage URLs rejected with 400/422 — input validation works; compose: smoke-contract/INVALID_URL_REJECTED: garbage URLs rejected with 400/422 — input vali… |
| teams-standard | Teams standard link (teams.microsoft.com/l/meetup-join/...) parses | 15 | ⬜ missing | lite: smoke-contract/TEAMS_URL_STANDARD: Teams standard join URL accepted by POST /bots; compose: smoke-contract/TEAMS_URL_STANDARD: Teams standard join URL accepted by POST /bots; helm: check TEAM… |
| teams-shortlink | Teams shortlink (teams.live.com, teams.microsoft.com/meet) parses | 10 | ⬜ missing | lite: smoke-contract/TEAMS_URL_SHORTLINK: Teams /meet/ shortlink URL parsed and accepted by POST /bots (no explicit platform needed); compose: smoke-contract/TEAMS_URL_SHORTLINK: Teams /meet/ short… |
| teams-channel | Teams channel meeting URL parses | 10 | ⬜ missing | lite: smoke-contract/TEAMS_URL_CHANNEL: Teams channel meeting URL accepted or known gap; compose: smoke-contract/TEAMS_URL_CHANNEL: Teams channel meeting URL accepted or known gap; helm: check TEAM… |
| teams-enterprise | Teams enterprise-tenant URL parses (custom domain) | 15 | ⬜ missing | lite: smoke-contract/TEAMS_URL_ENTERPRISE: Teams enterprise domain URL parsed and accepted by POST /bots (no explicit platform needed); compose: smoke-contract/TEAMS_URL_ENTERPRISE: Teams enterpris… |
| teams-personal | Teams personal-account URL parses | 15 | ⬜ missing | lite: smoke-contract/TEAMS_URL_PERSONAL: Teams personal (teams.live.com) URL parsed and accepted by POST /bots (no explicit platform needed); compose: smoke-contract/TEAMS_URL_PERSONAL: Teams perso… |
...
| lite-internal-services-loopback-only | deploy/lite/supervisord.conf binds every internal service (admin-api, runtime-api, meeting-api, agent-api, tts-service, mcp) to 127.0.0.1 | 10 | ❌ fail | lite: smoke-static/LITE_INTERNAL_SERVICES_LOOPBACK_ONLY: forbidden: host 0\.0\.0\.0 --port (8057\|8090\|8080\|8100\|8059\|18888) |
...
| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `dashboard-playback-canonical` | ✅ pass | 146 ms | 1 / 1 |
| `no-media-files-table-references` | ✅ pass | 1420 ms | 1 / 1 |
| `no-placeholder-transcription-token` | ✅ pass | 198 ms | 1 / 1 |
| `no-recordings-table-references` | ✅ pass | 1273 ms | 1 / 1 |
| `recording-finalizing-state` | ✅ pass | 132 ms | 1 / 1 |
| `recording-playback-url-canonical` | ✅ pass | 231 ms | 1 / 1 |
| `recordings-tables-dropped` | ✅ pass | 313 ms | 1 / 1 |
| `smoke-bot-transcription-roundtrip-roundtrip` | ✅ pass | 668 ms | 1 / 1 |
| `smoke-contract` | ✅ pass | 39998 ms | 27 / 27 |
| `smoke-env` | ✅ pass | 855 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 3833 ms | 13 / 17 |
| `smoke-static` | ❌ fail | 2650 ms | 90 / 92 |
| `v0.10.5.3-hallucination-corpus` | ✅ pass | 356 ms | 3 / 3 |
| `v0.10.6-static-greps` | ✅ pass | 1364 ms | 15 / 15 |
| `v0.10.6.1-arm64-manifest-arm64_in_manifest` | ✅ pass | 137 ms | 1 / 1 |
| `v0.10.6.1-dispatch-check-admin-deny-admin_bots_endpoint_honors_deny` | ✅ pass | 164 ms | 1 / 1 |
| `v0.10.6.1-dispatch-check-deny-bots_endpoint_honors_deny` | ✅ pass | 1202 ms | 1 / 1 |
| `v0.10.6.1-dispatch-check-noop-noop_when_unset` | ✅ pass | 264 ms | 1 / 1 |
| `v0.10.6.1-gmeet-fast-fail-eviction_retry_or_fail_clean` | ✅ pass | 133 ms | 1 / 1 |
| `v0.10.6.1-gmeet-fast-fail-rejection_under_30s` | ✅ pass | 135 ms | 1 / 1 |
| `v0.10.6.1-post-meeting-idempotency-fires_once_per_session` | ✅ pass | 170 ms | 1 / 1 |
| `v0.10.6.1-speak-defaults-defaults_propagate` | ✅ pass | 132 ms | 1 / 1 |
| `v0.10.6.1-speak-e2e-speak_delivers_audio` | ✅ pass | 207 ms | 1 / 1 |
| `v0.10.6.1-stale-audit-decisions_filed` | ✅ pass | 5680 ms | 1 / 1 |
| `v0.10.6.1-static-greps` | ✅ pass | 755 ms | 9 / 9 |
| `v0.10.6.1-teams-modal-modal_dismissed` | ✅ pass | 146 ms | 1 / 1 |
| `v0.10.6.1-tts-auto-lang-detects_and_picks_voice` | ✅ pass | 1812 ms | 1 / 1 |
| `v0.10.6.1-tts-auto-lang-voice_download_caches` | ✅ pass | 862 ms | 1 / 1 |
| `v0.10.6.1-tts-byo-playback-handles_wav_and_mp3` | ✅ pass | 140 ms | 1 / 1 |
| `v0.10.6.1-webhook-no-autotouch-no_autotouch` | ✅ pass | 107 ms | 1 / 1 |
| `v0.10.6.1-webm-duration-ebml_duration_present` | ✅ pass | 153 ms | 1 / 1 |
| `walkability-smoke-compose` | ✅ pass | 1099 ms | 6 / 6 |
...
| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `dashboard-playback-canonical` | ✅ pass | 149 ms | 1 / 1 |
| `no-media-files-table-references` | ✅ pass | 1401 ms | 1 / 1 |
| `no-placeholder-transcription-token` | ✅ pass | 211 ms | 1 / 1 |
| `no-recordings-table-references` | ✅ pass | 1293 ms | 1 / 1 |
| `smoke-bot-transcription-roundtrip-roundtrip` | ✅ pass | 664 ms | 1 / 1 |
| `smoke-contract` | ✅ pass | 38738 ms | 22 / 27 |
| `smoke-env` | ✅ pass | 637 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 4123 ms | 12 / 17 |
| `smoke-static` | ❌ fail | 2565 ms | 90 / 92 |
| `v0.10.5.3-hallucination-corpus` | ✅ pass | 351 ms | 3 / 3 |
| `v0.10.6-static-greps` | ✅ pass | 1304 ms | 15 / 15 |
| `v0.10.6.1-dispatch-check-noop-noop_when_unset` | ✅ pass | 278 ms | 1 / 1 |
| `v0.10.6.1-static-greps` | ✅ pass | 805 ms | 9 / 9 |
| `walkability-smoke-lite` | ✅ pass | 1165 ms | 6 / 6 |
...
| `scope-proof-gate-local` | ✅ pass | 354 ms | 1 / 1 |
