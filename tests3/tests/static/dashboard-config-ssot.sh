...
docker_default="$(rg -n 'ARG VEXA_API_URL=' "$ROOT_DIR/services/dashboard/Dockerfile" 2>/dev/null || true)"
...
if [ -z "$docker_default$entrypoint_patch" ]; then
  step_pass DASHBOARD_REWRITES_REQUIRE_BUILD_SSOT "dashboard image requires VEXA_API_URL build arg and does not patch rewrites at runtime"
else
  step_fail DASHBOARD_REWRITES_REQUIRE_BUILD_SSOT "$(printf '%s\n%s' "$docker_default" "$entrypoint_patch" | head -10 | tr '\n' ' ')"
fi
