#!/usr/bin/env bash
# Deploy compose on a freshly provisioned VM.
...
# ── 3. Clone repo ────────────────────────────────
# Idempotent: wipe any pre-existing /root/vexa first so a reused/dirty VM doesn't fail
# the clone ("destination already exists") and silently leave a stale checkout.
...
vm_ssh "cd /root/vexa/deploy/compose && make env" 2>&1 | tail -1
...
for ep in "8056:gateway" "8057:admin-api" "3001:dashboard"; do
    PORT=${ep%%:*}
    NAME=${ep##*:}
