...
| OSS-3 | CDP proxy: 307 scheme downgrade, Host-header DNS-rebind block | **Host-header rewrite is ALREADY IN PLACE** — `main.py:1819, 1854, 1879` all pass `Host: localhost` to upstream. Remaining defects: (a) `main.py:1824` hardcodes a plaintext WebSocket scheme for `proxy_ws_url` — doesn't preserve `wss://` for hosted deployments, (b) `@app.api_route(".../cdp/{path:path}", methods=["GET"])` at `main.py:1807` matches `/cdp/json/version` but not bare `/cdp`. **Valid but narrower than PRD framed it.** | **YES, reduced** |
...
- **scope**:
  1. `services/api-gateway/main.py:1824` — derive scheme from upstream forwarded headers (`X-Forwarded-Proto` / `request.url.scheme`) so `proxy_ws_url` becomes `wss://` on HTTPS gateways and a plaintext WebSocket only on local dev.
